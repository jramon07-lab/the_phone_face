...
async function processJob(job:any,secret=""){try{
  ...
  }catch(err:any){const msg=String(err?.message||err||"Error desconocido");if(err?.responseCheck){await auditFollowup(job,"verification_deferred","warning",msg);await requeue(job,msg,5,true);return "requeued";}if(err?.beforeSend&&Number(job.attempts||0)<10){await auditFollowup(job,"delivery_deferred","warning",msg);await requeue(job,msg,2);return "requeued";}await auditFollowup(job,"followup_failed","error",msg);await complete(job,"failed",msg);return "failed";}}
