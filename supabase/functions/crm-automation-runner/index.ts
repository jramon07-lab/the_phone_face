...
async function processJob(job:any,secret=""){try{
    ...
    if(job.action_type==="create_task"){
      ...
      if(!(await preflight(job)))return "requeued";const chatId=phoneToChat(job.context);const sent=await sendGreen(chatId,vars(String(a.text||""),job.context),secret,Array.isArray(a.reply_buttons)?a.reply_buttons:[]);await rememberOfferMessage(job,sent);await stageDeliveryVerification(job,chatId,sent.idMessage);return "requeued";
    }else if(job.action_type==="send_template"){
      const tpl=a.template_id?await resolveTemplateId(job.user_id,a.template_id):await resolveTemplate(job.user_id,Number(a.template_index||0));if(!(await preflight(job)))return "requeued";const chatId=phoneToChat(job.context);const sent=await sendGreen(chatId,vars(String(tpl.body||""),job.context),secret,Array.isArray(a.reply_buttons)?a.reply_buttons:[]);await rememberOfferMessage(job,sent);await stageDeliveryVerification(job,chatId,sent.idMessage);return "requeued";
