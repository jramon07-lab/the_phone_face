...
async function processJob(job:any,secret=""){try{
    ...
    const skipped=await shouldSkip(job,secret);if(skipped){await complete(job,"done",undefined,{skipped:true,skip_reason:skipped});return "done";}
    if(job.action_type==="create_task"){
      ...
      if(!(await preflight(job)))return "requeued";await sendGreen(phoneToChat(job.context),vars(String(a.text||""),job.context),secret,Array.isArray(a.reply_buttons)?a.reply_buttons:[]);
