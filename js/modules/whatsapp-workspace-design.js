...
function fit(){if(innerWidth<1051)return;const page=view.querySelector('.waLivePage');if(page&&view.getBoundingClientRect().height){const h=Math.max(360,Math.min(innerHeight,view.getBoundingClientRect().bottom)-page.getBoundingClientRect().top-8);if(page.style.height!==h+'px')page.style.setProperty('height',h+'px','important');}}
...
function install(){
 ...
 const tabs=view.querySelector('.waTabs');const overflow=document.createElement('details');overflow.className='waCleanMore waCleanFilters';overflow.innerHTML='<summary aria-label="Más filtros">Más</summary><div></div>';tabs.append(overflow);for(const b of [...tabs.querySelectorAll('[data-wa-tab]')])if(!['unanswered','waiting','automatic','all'].includes(b.dataset.waTab))overflow.lastChild.append(b);
 for(const key of ['unanswered','waiting','automatic','all']){const b=tabs.querySelector('[data-wa-tab="'+key+'"]');if(b){if(key==='all')b.textContent='Todos';tabs.insertBefore(b,overflow);}}
