...
function install(){
 ...
 const tabs=view.querySelector('.waTabs');const overflow=document.createElement('details');overflow.className='waCleanMore waCleanFilters';overflow.innerHTML='<summary aria-label="Más filtros">⚙ Más</summary><div></div>';tabs.append(overflow);for(const b of [...tabs.querySelectorAll('[data-wa-tab]')])if(!['all','automatic','unread','unanswered','archived'].includes(b.dataset.waTab))overflow.lastChild.append(b);
 for(const key of ['all','unread','unanswered','automatic','archived']){const b=tabs.querySelector('[data-wa-tab="'+key+'"]');if(b){if(key==='all')b.textContent='Todos';tabs.insertBefore(b,overflow);}}
...
function refresh(){
 ...
 window.TPFWhatsappOfferSummary(id).then(rows=>{if(token!==revision)return;const root=$('waCleanOfferRows');root.replaceChildren();$('waCleanOffers').firstChild.textContent='Ofertas y seguimiento · '+rows.length;for(const x of rows.slice(0,4)){const row=document.createElement('div');row.className='waCleanOfferRow';const title=document.createElement('b');title.textContent=x.title;const meta=document.createElement('small');meta.textContent=x.amount+' · '+x.status;row.append(title,meta);
const actions=document.createElement('div');actions.className='cpOfferActions';
const available=['queued','following','paused'].includes(x.rawStatus)?[[x.rawStatus==='paused'?'resume':'pause',x.rawStatus==='paused'?'Reanudar':'Pausar'],['accept','Aceptada'],['cancel','Finalizar']]:[];
for(const [action,label] of available){const b=button('',label,async()=>{if(String(contact()?.id||'')!==id)return;const buttons=[...root.querySelectorAll('button')];buttons.forEach(n=>n.disabled=true);try{await window.TPFControlWhatsappOffer(x.id,action)}finally{buttons.forEach(n=>n.disabled=false)}});b.style.fontSize='12px';if(action==='accept')b.className='primary';actions.append(b)}
row.append(actions);root.append(row);}if(!rows.length)root.textContent='Sin ofertas para este contacto.';}).catch(()=>{if(token===revision)$('waCleanOfferRows').textContent='No se pudieron cargar las ofertas.';});
}
