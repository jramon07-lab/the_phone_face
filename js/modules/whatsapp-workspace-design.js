...
let activeId='',revision=0,queued=false,full=false,sideTab='client';
...
function organizeSide(){
 ...
 const offers=$('waCleanOffers'),work=$('waSidePanel-work');if(offers&&work.firstElementChild!==offers)work.prepend(offers);if(offers&&!offers.dataset.tabsOpened){offers.dataset.tabsOpened='1';offers.open=true;}
...
function refresh(){
 ...
 organizeSide();fit();if(!window.TPFContactRelations?.sidebarSnapshot||!window.TPFWhatsappOfferSummary){setTimeout(schedule,500);return;}if(id===activeId)return;activeId=id;const token=++revision;
...
root.append(row);}if(!rows.length)root.textContent='Sin ofertas para este contacto.';}).catch(()=>{if(token===revision)$('waCleanOfferRows').textContent='No se pudieron cargar las ofertas.';});
}
window.addEventListener('tpf:sales-updated',()=>{activeId='';schedule()});
