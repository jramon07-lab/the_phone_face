...
function modal(){
 ...
 $('gcaRows').onclick=event=>{const button=event.target.closest('[data-gca-action]');if(!button)return;const row=state.rows.find(item=>rowKey(item)===button.dataset.gcaKey);if(!row)return;if(button.dataset.gcaAction==='view'){showComparison(row);return}setDecision(row,button.dataset.gcaAction)};
