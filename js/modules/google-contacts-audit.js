...
const state={rows:[],filter:'all',query:'',page:1,running:false};
...
function modal(){
 ...
 back.querySelectorAll('[data-gca-filter]').forEach(button=>button.onclick=()=>{state.filter=button.dataset.gcaFilter;state.page=1;render()});$('gcaSearch').oninput=event=>{state.query=event.target.value;state.page=1;render()};$('gcaPrev').onclick=()=>{state.page=Math.max(1,state.page-1);render()};$('gcaNext').onclick=()=>{state.page++;render()};return back;
}
async function run(){
...
function styles(){if($('gcaStyles'))return;const style=document.createElement('style');style.id='gcaStyles';style.textContent=`.gcaSettingsBox{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-top:16px;padding-top:14px;border-top:1px solid #e3e8ef}.gcaSettingsBox p{margin:3px 0}.gcaBack{position:fixed;inset:0;z-index:260000;display:grid;place-items:center;padding:18px;background:#101828b8}.gcaBack.hidden{display:none}.gcaModal{width:min(1180px,100%);max-height:94vh;display:flex;flex-direction:column;overflow:hidden;border-radius:16px;background:#fff;box-shadow:0 28px 90px #0006}.gcaModal>header{display:flex;justify-content:space-between;gap:14px;padding:18px 20px;border-bottom:1px solid #e4e8ef}.gcaModal header span{color:#175cd3;font-size:10px;font-weight:850}.gcaModal h2{margin:3px 0}.gcaModal header p{margin:0;color:#667085;font-size:12px}.gcaModal header button{width:36px;height:36px;border:0;border-radius:50%;font-size:22px}.gcaLoading,.gcaError{margin:22px;padding:24px;border-radius:12px;background:#f5f8fc}.gcaLoading span{display:block;margin-top:5px;color:#667085}.gcaError{color:#9b2737;background:#fff0f1}.gcaStats{display:grid;grid-template-columns:repeat(6,1fr);gap:8px;padding:14px 18px}.gcaStats button{padding:10px;border:1px solid #dfe5ed;border-radius:10px;background:#fff;text-align:left}.gcaStats button.active{border-color:#175cd3;background:#eff5ff}.gcaStats span{display:block;color:#667085;font-size:9px}.gcaStats b{font-size:20px}.gcaTools{display:flex;align-items:center;gap:12px;padding:0 18px 12px}.gcaTools input{flex:1}.gcaTools span{color:#667085;font-size:11px}.gcaTableWrap{overflow:auto;padding:0 18px;min-height:250px}.gcaTableWrap table{width:100%;border-collapse:collapse;min-width:900px}.gcaTableWrap th,.gcaTableWrap td{padding:10px;border-bottom:1px solid #e8ecf2;text-align:left;vertical-align:top;font-size:11px}.gcaTableWrap th{position:sticky;top:0;background:#f8fafc;color:#667085;text-transform:uppercase}.gcaTableWrap td small{display:block;color:#667085;margin-top:3px}.gcaBadge{display:inline-block;padding:4px 7px;border-radius:999px;background:#eef2f6}.gcaBadge.match{background:#e5f7eb;color:#14753a}.gcaBadge.conflict,.gcaBadge.duplicate{background:#fff0e0;color:#a64b00}.gcaBadge.google_only{background:#eef4ff;color:#1849a9}.gcaBadge.crm_only{background:#f2ebff;color:#6941c6}.gcaEmpty{text-align:center!important;padding:40px!important;color:#667085}.gcaModal>div.hidden{display:none}.gcaModal>footer,.gcaContent+footer{display:none}.gcaContent{}.gcaModal #gcaContent>footer{display:flex;align-items:center;justify-content:center;gap:14px;padding:12px 18px;border-top:1px solid #e4e8ef}@media(max-width:800px){.gcaSettingsBox{align-items:flex-start;flex-direction:column}.gcaStats{grid-template-columns:repeat(2,1fr)}.gcaBack{padding:6px}.gcaModal{max-height:98vh}.gcaTools{align-items:stretch;flex-direction:column}}`;document.head.appendChild(style)}
function bind(){styles();const button=$('googleContactsAuditBtn');if(button)button.onclick=run;window.TPFGoogleContactsAudit={compare,run}}
