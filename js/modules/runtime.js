(function(){
  ...
  function safeString(v){
    try{return String(v??'');}catch(_){return 'Error desconocido';}
  }
  ...
  function persistError(entry){
    try{
      ...
      prev.unshift(entry);
      localStorage.setItem(ERROR_KEY,JSON.stringify(prev.slice(0,MAX_ERRORS)));
    }catch(_){ }
  }

  function emit(name,state,detail=''){
    const item={name,state,detail:safeString(detail),at:new Date().toISOString()};
    states.set(name,item);
    try{window.dispatchEvent(new CustomEvent('tpf:module-status',{detail:item}));}catch(_){ }
    return item;
  }

  function report(name,error,context=''){
    const message=safeString(error?.message||error);
    const entry={module:name,message,context:safeString(context),at:new Date().toISOString()};
    persistError(entry);
    emit(name,'error',message);
    try{console.error(`[TPF:${name}]`,context||'error',error);}catch(_){ }
    return entry;
  }

  function guard(name,fn,options={}){
    if(typeof fn!=='function') return fn;
    if(fn.__tpfGuarded) return fn;
    const rethrow=options.rethrow===true;
    const fallback=Object.prototype.hasOwnProperty.call(options,'fallback')?options.fallback:undefined;
    const wrapped=function(...args){
      try{
        const result=fn.apply(this,args);
        if(result && typeof result.then==='function'){
          return result.then(value=>{emit(name,'ok');return value;}).catch(error=>{
            report(name,error,fn.name||'async');
        ...
        report(name,error,fn.name||'sync');
        if(rethrow) throw error;
        return typeof fallback==='function'?fallback(error):fallback;
      }
    };
    Object.defineProperty(wrapped,'__tpfGuarded',{value:true});
    Object.defineProperty(wrapped,'__tpfOriginal',{value:fn});
    return wrapped;
  }

  function wrapGlobals(name,names,options={}){
    const rethrowSet=new Set(options.rethrow||[]);
    const wrapped=[];
    for(const key of names||[]){
      const fn=window[key];
      if(typeof fn!=='function' || fn.__tpfGuarded) continue;
      window[key]=guard(name,fn,{rethrow:rethrowSet.has(key)});
      wrapped.push(key);
    }
    return wrapped;
  }
  ...
  function register(name,definition={}){
    ...
    if(states.get(name)?.state==='ready' || states.get(name)?.state==='ok') return states.get(name);
    emit(name,'loading');
    try{
      if(typeof definition.install==='function') definition.install(api);
      return emit(name,'ready');
    }catch(error){
      report(name,error,'install');
