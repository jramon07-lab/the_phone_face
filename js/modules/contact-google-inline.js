...
function openCorrection(options={}){
 ...
 $('tpfInlineUseGoogle').disabled=!available.length;$('tpfInlineUseGoogle').onclick=()=>{const g=googleView(available[Number(select.value)]||available[0]);apply(g.first,g.last,g.nickname)};
...
async function saveCorrection(){
 if(busy)return;const row=correctionRow||current(),available=correctionMatches,c=contactData(row),first=safe($('tpfInlineFirst').value),last=safe($('tpfInlineLast').value),nickname=safe($('tpfInlineNickname').value),full=[first,last].filter(Boolean).join(' '),msg=$('tpfInlineMsg'),btn=$('tpfInlineSave'),selection=safe($('tpfInlineGoogle').value);if(!row||!first){msg.textContent='Escribe al menos el nombre.';return}if(!safe(sessionStorage.getItem('tpf_google_contacts_token'))){msg.textContent='Conecta Google Contacts antes de guardar en los tres sitios.';return}if(correctionGoogleError){msg.textContent='No se guardará: vuelve a conectar Google y compruébalo otra vez.';return}if(!available.length){msg.textContent='No se guardará ni se creará otro contacto hasta comprobar Google.';return}if(available.length>1&&!selection){msg.textContent='Elige cuál de los contactos duplicados de Google quieres conservar.';return}
 busy=true;btn.disabled=true;msg.textContent='Guardando…';try{await writeCrm(row,first,last,nickname);const person=available[Number(selection)];await writeGoogle(person,{...c,name:full},first,last,nickname);msg.textContent='Nombre, apellidos y apodo guardados en CRM y en el contacto de Google elegido.';window.dispatchEvent(new CustomEvent('tpf:contact-updated',{detail:{id:row.id}}));setTimeout(()=>{$('tpfInlineBack').classList.add('hidden');if(correctionReturn==='whatsapp'){waSignature='';refreshWhatsapp()}else window.openContact?.(row.id)},550)}catch(e){msg.textContent=e?.message||'No se pudo guardar.'}finally{busy=false;btn.disabled=false}
}
