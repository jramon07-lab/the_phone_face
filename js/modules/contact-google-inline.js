...
function ensureModal(){let back=$('tpfInlineBack');if(back)return back;back=document.createElement('div');back.id='tpfInlineBack';back.className='tpfInlineBack hidden';back.innerHTML=`<section class="tpfInlineModal" role="dialog" aria-modal="true"><header><div><small>UNIFICAR CONTACTO</small><h3>Cómo quedará en los tres sitios</h3></div><button id="tpfInlineClose" type="button">×</button></header><div class="tpfInlineBody"><div id="tpfInlineSource" class="tpfInlineSource"></div><div class="tpfInlineChoices"><button id="tpfInlineUseCrm" class="secondary" type="button">Usar datos del CRM</button><button id="tpfInlineUseGoogle" class="secondary" type="button">Usar datos de Google</button><button id="tpfInlineUseWhatsapp" class="secondary" type="button">Usar WhatsApp como apodo</button></div><div class="tpfInlineGrid"><label class="full">Contacto de Google<select id="tpfInlineGoogle"></select></label><label>Nombre<input id="tpfInlineFirst" autocomplete="given-name"></label><label>Apellidos<input id="tpfInlineLast" autocomplete="family-name"></label><label class="full">Apodo visible<input id="tpfInlineNickname" placeholder="Nombre que quieres ver en llamadas"></label></div><div id="tpfInlinePreview" class="tpfInlinePreview"></div><div id="tpfInlineMsg" class="tpfInlineMsg"></div></div><footer><button id="tpfInlineIgnore" class="secondary hidden" type="button">Ignorar este nombre</button><button id="tpfInlineCancel" class="secondary" type="button">Cerrar sin cambios</button><button id="tpfInlineSave" class="primary" type="button">Guardar unificado</button></footer></section>`;document.body.appendChild(back);const close=()=>back.classList.add('hidden');$('tpfInlineClose').onclick=close;$('tpfInlineCancel').onclick=close;back.onclick=e=>{if(e.target===back)close()};$('tpfInlineSave').onclick=saveCorrection;['tpfInlineFirst','tpfInlineLast','tpfInlineNickname'].forEach(id=>$(id).addEventListener('input',renderFinalPreview));return back}
function splitName(name){const parts=safe(name).split(/\s+/).filter(Boolean);return{first:parts.shift()||'',last:parts.join(' ')}}
function renderFinalPreview(){const first=safe($('tpfInlineFirst')?.value),last=safe($('tpfInlineLast')?.value),nickname=safe($('tpfInlineNickname')?.value),full=[first,last].filter(Boolean).join(' ')||'Sin nombre',visible=nickname||full;const box=$('tpfInlinePreview');if(!box)return;box.innerHTML=`<h4>Así quedará al guardar</h4><p>WhatsApp dentro del CRM: <b>${esc(visible)}</b></p><p>Ficha del CRM: <b>${esc(full)}</b>${nickname?` · Apodo: <b>${esc(nickname)}</b>`:''}</p><p>Google y llamadas: <b>${esc(visible)}</b></p><small>El nombre público que tenga la otra persona en WhatsApp no se puede cambiar; dentro del CRM sí se mostrará el nombre unificado.</small>`}
function openCorrection(options={}){
 ...
 const apply=(first,last,nick)=>{$('tpfInlineFirst').value=first;$('tpfInlineLast').value=last;$('tpfInlineNickname').value=nick;renderFinalPreview()};
 $('tpfInlineUseCrm').onclick=()=>apply(field(row?.data||{},'NOMBRE')||parts.first,field(row?.data||{},'APELLIDOS','APELLIDO')||parts.last,c.nickname);
 $('tpfInlineUseGoogle').disabled=!available.length;$('tpfInlineUseGoogle').onclick=()=>{const g=googleView(available[Number(select.value)]||available[0]),p=splitName(g.name);apply(p.first,p.last,g.nickname)};
 $('tpfInlineUseWhatsapp').disabled=!validWaName(wa);$('tpfInlineUseWhatsapp').onclick=()=>apply(safe($('tpfInlineFirst').value),safe($('tpfInlineLast').value),wa);
