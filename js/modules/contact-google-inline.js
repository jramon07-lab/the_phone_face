...
function ensureModal(){let back=$('tpfInlineBack');if(back)return back;back=document.createElement('div');back.id='tpfInlineBack';back.className='tpfInlineBack hidden';back.innerHTML=`<section class="tpfInlineModal" role="dialog" aria-modal="true"><header><div><small>UNIFICAR CONTACTO</small><h3>El mismo nombre en los tres sitios</h3></div><button id="tpfInlineClose" type="button">×</button></header><div class="tpfInlineBody"><div id="tpfInlineSource" class="tpfInlineSource"></div><div class="tpfInlineChoices"><button id="tpfInlineUseCrm" class="secondary" type="button">Usar nombre del CRM</button><button id="tpfInlineUseGoogle" class="secondary" type="button">Usar nombre de Google</button><button id="tpfInlineUseWhatsapp" class="secondary" type="button">Usar nombre de WhatsApp</button></div><div class="tpfInlineGrid"><label class="full">Contacto de Google<select id="tpfInlineGoogle"></select></label><label class="full">Nombre final para todos<input id="tpfInlineFinalName" autocomplete="name" placeholder="Nombre que aparecerá en todas partes"></label></div><div id="tpfInlinePreview" class="tpfInlinePreview"></div><div id="tpfInlineMsg" class="tpfInlineMsg"></div></div><footer><button id="tpfInlineIgnore" class="secondary hidden" type="button">Ignorar este nombre</button><button id="tpfInlineCancel" class="secondary" type="button">Cerrar sin cambios</button><button id="tpfInlineSave" class="primary" type="button">Guardar en los tres</button></footer></section>`;document.body.appendChild(back);const close=()=>back.classList.add('hidden');$('tpfInlineClose').onclick=close;$('tpfInlineCancel').onclick=close;back.onclick=e=>{if(e.target===back)close()};$('tpfInlineSave').onclick=saveCorrection;$('tpfInlineFinalName').addEventListener('input',renderFinalPreview);return back}
