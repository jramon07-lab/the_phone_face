...
function openCorrection(options={}){
 ...
 correctionRow=row;correctionMatches=available;correctionWhatsapp=wa;correctionChat=contactChat(row,options.chat||selectedWa());correctionGoogleError=safe(options.googleError);correctionReturn=options.returnTo||'profile';correctionHolder=null;holderResults=[];
...
async function writeCrm(row,first,last,nickname,chat,verifiedGoogle){
 ...
 d.NOMBRE=normalizedFirst;d.APELLIDOS=normalizedLast;d['NOMBRE Y APELLIDOS']=name;d.APODO=normalizedNickname;
...
async function createSeparateCrm(first,last,nickname,holder,chat,verifiedGoogle){
 if(typeof perms!=='undefined'&&!(perms?.is_admin||perms?.can_create_database))throw Error('No tienes permiso para crear contactos.');
 const existing=await boundContact(chat);if(existing&&safe(existing.id)!==safe(holder.record_id)){await writeCrm(existing,first,last,nickname,chat,verifiedGoogle);return existing}
...
async function saveCorrection(){
 ...
 try{
  ...
  const savedRow=separate?await createSeparateCrm(first,last,nickname,correctionHolder,correctionChat,verifiedGoogle):row;
  if(!separate)await writeCrm(row,first,last,nickname,correctionChat,verifiedGoogle);else rememberBinding(correctionChat,savedRow);rememberBinding(correctionChat,savedRow);rememberUnifiedName(correctionChat,savedRow);
