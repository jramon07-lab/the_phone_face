...
function open(button,id){
 ...
 menu.innerHTML='<button data-contact-more="open">Abrir ficha</button><button data-contact-more="edit">Editar datos</button><button data-contact-more="labels">Gestionar etiquetas</button><button data-contact-more="opportunity">Crear oportunidad</button><button data-contact-more="task">Crear tarea</button><button data-contact-more="delete" class="danger">Eliminar contacto</button>';
...
M.register('contacts-more-menu',{install(){style();document.addEventListener('pointerdown',mark,true);document.addEventListener('click',click,true);document.addEventListener('click',event=>{if(!event.target.closest?.('.tpfContactsMoreMenu,.tpfContactActions'))close();},true);}});
})();
