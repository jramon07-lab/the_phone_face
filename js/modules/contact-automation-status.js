...
const ACTION_NAMES={flow_v1:'Inicio de automatización',create_opportunity:'Crear oportunidad',create_task:'Crear tarea',send_template:'Enviar plantilla de WhatsApp',send_whatsapp_now:'Enviar WhatsApp',__send_whatsapp:'Enviar WhatsApp',assign_label:'Asignar etiqueta',move_opportunity:'Mover oportunidad',schedule_whatsapp:'Programar WhatsApp',record_offer_month:'Registrar oferta',record_sale_month:'Registrar venta',set_review_date:'Fijar fecha de revisión',prepare_operator_review:'Preparar revisión de renovación con el operador'};
const TRIGGER_NAMES={message_received:'Cuando llega un WhatsApp',message_contains:'Cuando un WhatsApp contiene una palabra',opportunity_stage:'Cuando la oportunidad cambia de columna',label_assigned:'Cuando se asigna una etiqueta',unanswered:'Cuando el cliente no responde'};
...
async function fetchSnapshot(contact,force=false){
 ...
 const request=(async()=>{
  const [automationResult,jobsResult,templatesResult,labelsResult,stagesResult]=await Promise.all([
   ...
   sb.from('crm_labels').select('id,name').limit(500),
   sb.from('sales_stages').select('id,name').limit(500)
  ]);
  ...
  const labels=new Map((labelsResult.error?[]:labelsResult.data||[]).map(row=>[String(row.id),row]));
  const stages=new Map((stagesResult.error?[]:stagesResult.data||[]).map(row=>[String(row.id),row]));
  const snapshot={contactId:id,contactName:contactName(contact),executions,summary:summarize(executions),templates,labels,stages,loadedAt:Date.now()};cache.set(id,snapshot);return snapshot;
...
function stepState(job,fallback='pending'){if(!job)return fallback;const status=String(job.status||'').toLowerCase();if(status==='done'||status==='completed')return'completed';if(status==='failed'||status==='error')return'error';if(status==='cancelled')return'cancelled';if(status==='running')return'running';if(status==='paused')return'paused';return'pending'}
function stepBadge(status){return badge(status==='completed'?'completed':status)}
function stepDetails(type,config={},snapshot){
 const rows=[],add=(label,value)=>{if(value!==undefined&&value!==null&&String(value).trim()!=='')rows.push([label,String(value)])};
 if(type==='assign_label'){const label=snapshot.labels?.get(String(config.label_id||''));add('Etiqueta',config.resolved_label_name||label?.name||config.label_name_template||config.label_id);add('Resultado','Se añadirá a la ficha del cliente')}
 else if(type==='create_task'){add('Tarea',config.title);add('Descripción',config.description);add('Estado inicial',config.status||'Pendiente')}
 else if(type==='create_opportunity'){const stage=snapshot.stages?.get(String(config.stage_id||''));add('Oportunidad',config.title);add('Importe',config.amount!=null?config.amount+' €':'');add('Columna',stage?.name||config.stage_id);add('Estado',config.status)}
 else if(type==='move_opportunity'){const stage=snapshot.stages?.get(String(config.stage_id||''));add('Nueva columna',stage?.name||config.stage_id)}
 else if(type==='send_template'){const template=snapshot.templates?.get(String(config.template_id||''));add('Plantilla',template?.name||config.template_name||config.template_id);add('Mensaje',template?.body||config.body||config.text)}
 else if(type==='send_whatsapp_now'||type==='__send_whatsapp'||type==='schedule_whatsapp')add('Mensaje',config.text||config.message);
 else if(type==='set_review_date'){add('Revisión dentro de',`${config.value||1} ${unit(config.unit||'years')}`)}
 else if(type==='prepare_operator_review'){add('Acción','Preparar la revisión de renovación del cliente');add('Nombre de la revisión',config.title);const stage=snapshot.stages?.get(String(config.stage_id||''));add('Columna',stage?.name||config.stage_id);add('Resultado','Creará el aviso o registro necesario para revisar la renovación')}
 else if(type==='record_offer_month')add('Resultado','Registrará la oferta en el mes correspondiente');
 else if(type==='record_sale_month')add('Resultado','Registrará la venta en el mes correspondiente');
 return rows;
}
function executionSteps(item,snapshot){
 const root=item.jobs.find(job=>job.action_type==='flow_v1')||item.jobs[0],configured=root?.action_config?.steps||item.automation?.action_config?.steps,steps=[];
 if(root?.action_type==='flow_v1')steps.push({title:'Inicio de la automatización',status:stepState(root,'completed'),at:root.completed_at||root.created_at,details:[['Condición',TRIGGER_NAMES[item.automation?.trigger_type]||root.context?.trigger_type||'Inicio programado']]});
 if(Array.isArray(configured))configured.forEach((step,index)=>{
  if(step?.kind==='action'){
   const job=item.jobs.find(row=>row.action_type!=='flow_v1'&&Number(row.action_config?.__flow_step)===index+1),config=job?.action_config||step.config||{};
   steps.push({title:ACTION_NAMES[step.action_type]||step.action_type||'Acción',status:stepState(job,item.status==='cancelled'?'cancelled':'pending'),at:job?.completed_at||job?.run_at,details:stepDetails(step.action_type,config,snapshot),error:job?.error_message||''});
  }else if(step?.kind==='wait'){
   const next=item.jobs.find(row=>row.action_type!=='flow_v1'&&Number(row.action_config?.__flow_step)>index+1),future=next&&String(next.status)==='pending'&&ts(next.run_at)>Date.now();
   steps.push({title:`Esperar ${step.value||0} ${unit(step.unit)}`,status:item.status==='cancelled'?'cancelled':future?'pending':'completed',at:next?.run_at,details:[[future?'Continuará':'Resultado',future?fmt(next.run_at):'Espera completada']]});
  }else if(step?.kind==='condition')steps.push({title:'Comprobar condición',status:item.status==='cancelled'?'cancelled':'completed',details:[['Condición',step.condition_type==='no_response'?'El cliente no ha respondido':'Condición configurada']]});
  else if(step?.kind==='repeat')steps.push({title:'Repetición programada',status:item.status==='cancelled'?'cancelled':'completed',details:[['Frecuencia',`Cada ${step.every_value||''} ${unit(step.every_unit)} · ${step.times||''} veces`]]});
 });
 if(!Array.isArray(configured))for(const job of item.jobs)if(job!==root||root.action_type!=='flow_v1')steps.push({title:ACTION_NAMES[job.action_type]||job.action_type||'Acción',status:stepState(job),at:job.completed_at||job.run_at,details:stepDetails(job.action_type,job.action_config||{},snapshot),error:job.error_message||''});
 return steps;
}
function stepRows(item,snapshot){const steps=executionSteps(item,snapshot);return `<div class="casSteps"><b>Pasos de esta automatización</b>${steps.map((step,index)=>`<div class="casStep ${esc(step.status)}"><span class="casStepNumber">${index+1}</span><div><strong>${esc(step.title)}</strong>${step.at?`<small>${esc(fmt(step.at))}</small>`:''}${step.details?.length?`<dl>${step.details.map(([label,value])=>`<div><dt>${esc(label)}</dt><dd>${esc(value)}</dd></div>`).join('')}</dl>`:''}${step.error?`<p class="casStepError">${esc(step.error)}</p>`:''}</div>${stepBadge(step.status)}</div>`).join('')}</div>`}
...
window.TPFContactAutomationStatus={rootOf,executionStatus,groupJobs,summarize,stepDetails,executionSteps,fetchSnapshot,refresh:()=>refreshVisible(true),open:contact=>openDialog(contact)};
