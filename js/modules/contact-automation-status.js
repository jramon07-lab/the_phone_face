...
function summaryHtml(snapshot,compact=false){
 ...
 return `<div class="casHead"><b>Automatizaciones</b><button type="button" class="casRefresh" data-cas-refresh title="Actualizar" aria-label="Actualizar automatizaciones">↻</button></div>
 <div class="casMetrics"><span class="active"><b>${s.active}</b> activas</span><span class="scheduled"><b>${s.scheduled}</b> programadas</span><span class="completed"><b>${s.completed}</b> completadas</span><span class="${s.errors?'errors':'clear'}"><b>${s.errors}</b> errores</span></div>
 ...
 <button type="button" class="casOpen" data-cas-open>Ver detalle</button>`;
}
...
async function fetchSnapshot(contact,force=false){
 ...
 const request=(async()=>{
  const [automationResult,jobsResult]=await Promise.all([
  ...
  const snapshot={contactId:id,contactName:contactName(contact),executions,summary:summarize(executions),loadedAt:Date.now()};cache.set(id,snapshot);return snapshot;
...
async function renderTarget(target,contact,force=false){
 ...
 try{const snapshot=await fetchSnapshot(contact,force);if(!targetStillMatches(target,id)||box.dataset.contactId!==id)return;box.innerHTML=summaryHtml(snapshot,target==='whatsapp')}
...
function detailRows(snapshot){return snapshot.executions.map(item=>`<article class="casExecution ${esc(item.status)}"><div class="casExecutionTop"><div><b>${esc(automationLabel(item))}</b><small>${esc(fmt(item.startedAt))}</small></div>${badge(item.status)}</div><div class="casExecutionInfo">${item.nextJob?`<span>Próximo paso</span><b>${esc(actionName(item.nextJob))} · ${esc(fmt(item.nextJob.run_at))}</b>`:item.failedJob?`<span>Fallo registrado</span><b class="error">${esc(item.failedJob.error_message||actionName(item.failedJob))}</b>`:'<span>Estado</span><b>Sin pasos pendientes</b>'}</div><div class="casExecutionActions">${['pending','running'].includes(item.status)?`<button type="button" class="danger" data-cas-cancel="${esc(item.key)}">Cancelar seguimiento</button>`:''}${item.pausedJob?`<button type="button" data-cas-resume="${esc(item.pausedJob.id)}">Reanudar</button>`:''}${item.failedJob&&SAFE_RETRY.has(item.failedJob.action_type)?`<button type="button" data-cas-retry="${esc(item.failedJob.id)}">Reintentar paso</button>`:''}</div></article>`).join('')||'<div class="casEmpty large">No hay automatizaciones registradas para este cliente.</div>'}
