...
window.TPFContactRelations={async sidebarSnapshot(c){const [holders,managedBy]=await Promise.all([currentHolders(links(c.data?.TPF_RELACIONES)),managers(c.id)]);return {holders,managers:managedBy.map(identity),legacy:c.data?.TPF_TITULAR?.same===false?c.data.TPF_TITULAR.holder_name:null};},contactFingerprint:root=>JSON.stringify(forms.get(root)?.items||[]),opportunityContacts,opportunityPreview,prepareOpportunity,applyContactData,identity,links,match,searchRecords,record};
})();
