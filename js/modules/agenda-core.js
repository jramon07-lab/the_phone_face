...
async function loadAgenda(){
  ...
  syncAgendaFilterUi();
  const [one,two]=await Promise.all([q,sb.from("agenda_items").select("id,status,starts_at").or("whatsapp_enabled.is.null,whatsapp_enabled.eq.false").limit(1000)]);
  if(one.error){$("agendaList").innerHTML=`<div class="agendaEmpty">${esc(one.error.message)}</div>`;return}
  const rows=visibleRows(one.data||[]);
