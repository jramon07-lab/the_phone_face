...
function ensureUi(){
  ...
  const registry=document.createElement('div');registry.id='tpfIncidentRegistry';registry.className='card';registry.innerHTML='<div class="systemStatusHead"><div><h3>Registro diario de incidencias</h3><p class="small">Errores detectados en ordenador y móvil, compartidos entre dispositivos.</p></div><button id="tpfIncidentRefresh" class="secondary" type="button">Actualizar registro</button></div><div id="tpfIncidentMetrics" class="tpfIncidentMetrics"></div><div class="tpfIncidentFilters"><select id="tpfIncidentDays" aria-label="Periodo"><option value="1">Hoy</option><option value="7" selected>7 días</option><option value="30">30 días</option><option value="90">90 días</option><option value="0">Todo</option></select><select id="tpfIncidentStatus" aria-label="Estado"><option value="active" selected>Activas</option><option value="all">Todos los estados</option><option value="resolved">Resueltas</option><option value="ignored">Ignoradas</option></select><input id="tpfIncidentSearch" type="search" placeholder="Buscar error, módulo o dispositivo…" aria-label="Buscar incidencias"></div><div id="tpfIncidentList" class="tpfIncidentTableWrap"><div class="systemEmpty">Cargando incidencias…</div></div><p class="systemCoverageNote">Se registran fallos técnicos detectables (JavaScript, red, APIs, guardados, permisos, WhatsApp, automatizaciones y OCR). Un fallo puramente visual puede no generar error; para eso usa “Informar de un problema”.</p>';
