...
const INTERNAL_RPC=/crm_(?:report|list|set|cleanup|delete|clear)[\w]*system_event|crm_system_health_snapshot|crm_(?:list_offer_followup_events|offer_followup_latest)/i;
let rows=[],followups=[],health=null,green={state:null,health:null},loading=false,lastAction='',searchTimer=null,storageInfo=null;
...
async function resolveRecoveredRequest(recovery={}){
  ...
  const key=`${target}|${build}|${currentDevice}`,now=Date.now();if(now-Number(recoveryChecks.get(key)||0)<30000)return;recoveryChecks.set(key,now);
  try{
...
function ensureUi(){
  ...
  const followup=document.createElement('div');followup.id='tpfFollowupRegistry';followup.className='card';followup.innerHTML='<div class="systemStatusHead"><div><h3>Seguimientos de WhatsApp</h3><p class="small">Respuestas, cancelaciones y protecciones de las ofertas de 2 y 5 días.</p></div><span id="tpfFollowupChecked" class="small">Pendiente</span></div><div id="tpfFollowupMetrics" class="tpfIncidentMetrics"></div><div id="tpfFollowupList" class="tpfIncidentTableWrap"><div class="systemEmpty">Cargando seguimientos…</div></div><p class="systemCoverageNote">El registro es informativo: nunca mueve la oportunidad ni cambia su columna. Una respuesta cancela el seguimiento; si WhatsApp no puede verificarse, el envío se aplaza y queda registrado.</p>';
  const registry=document.createElement('div');registry.id='tpfIncidentRegistry';registry.className='card';registry.innerHTML='<div class="systemStatusHead"><div><h3>Registro diario de incidencias</h3><p class="small">Errores detectados en ordenador y móvil, compartidos entre dispositivos.</p></div><button id="tpfIncidentRefresh" class="secondary" type="button">Actualizar registro</button></div><div id="tpfIncidentMetrics" class="tpfIncidentMetrics"></div><div class="tpfIncidentFilters"><select id="tpfIncidentDays" aria-label="Periodo"><option value="1">Hoy</option><option value="7" selected>7 días</option><option value="30">30 días</option><option value="90">90 días</option><option value="0">Todo</option></select><select id="tpfIncidentStatus" aria-label="Estado"><option value="active" selected>Activas</option><option value="all">Todos los estados</option><option value="resolved">Resueltas</option><option value="ignored">Revisadas</option></select><select id="tpfIncidentVersion" aria-label="Versión"><option value="all">Todas las versiones</option><option value="current">Versión actual</option><option value="other">Otras versiones</option><option value="unknown">Sin versión registrada</option></select><input id="tpfIncidentSearch" type="search" placeholder="Buscar error, módulo o dispositivo…" aria-label="Buscar incidencias"></div><div id="tpfIncidentList" class="tpfIncidentTableWrap"><div class="systemEmpty">Cargando incidencias…</div></div><p class="systemCoverageNote">Se registran fallos técnicos detectables (JavaScript, red, APIs, guardados, permisos, WhatsApp, automatizaciones y OCR). Marcar revisada solo archiva el aviso, no repara la causa. Si el fallo se repite aparecerá de nuevo. Las incidencias resueltas se conservan en el historial. Un fallo puramente visual puede no generar error; para eso usa “Informar de un problema”.</p>';
  const maintenance=document.createElement('div');maintenance.id='tpfMaintenanceCard';maintenance.className='card';maintenance.innerHTML='<div class="systemStatusHead"><div><h3>Mantenimiento seguro</h3><p class="small">Libera caché y temporales técnicos sin tocar ningún dato comercial.</p></div><button id="tpfMaintenanceScan" class="secondary" type="button">Analizar espacio</button></div><div class="tpfMaintenanceGrid"><div class="tpfMaintenanceBox"><span>Caché y temporales de este navegador</span><b id="tpfBrowserStorage">Analizando…</b><small id="tpfBrowserStorageDetail">Solo recursos y copias técnicas regenerables.</small><button id="tpfCleanBrowser" class="secondary" type="button">Limpiar caché segura</button></div><div class="tpfMaintenanceBox"><span>Historial técnico central</span><b>Control manual protegido</b><small>Vacía solo incidencias resueltas o ignoradas. Las activas siempre se conservan.</small><button id="tpfCleanCentral" class="secondary tpfDangerAction" type="button">Vaciar historial cerrado</button></div><div class="tpfMaintenanceBox"><span>Temporales del servidor</span><b>Limpieza automática</b><small>Vercel no los conserva entre ejecuciones, por lo que no se acumulan ni requieren borrado manual.</small></div></div><div class="tpfProtectedData"><b>Siempre protegido:</b> contactos, oportunidades, tareas, agenda, chats, archivos de clientes, plantillas, usuarios, sesiones y configuración.</div><div id="tpfMaintenanceResult" class="small" aria-live="polite"></div>';
  anchor?.after(operations);operations.after(followup);followup.after(registry);registry.after(maintenance);
...
function statusLabel(value){return({active:'Activa',resolved:'Resuelta',ignored:'Revisada'})[value]||value}
...
function sameBuild(a,b){a=String(a||'').toLowerCase();b=String(b||'').toLowerCase();return a.length>=7&&b.length>=7&&(a.startsWith(b)||b.startsWith(a))}
function renderRows(){
  const metrics=$('tpfIncidentMetrics'),list=$('tpfIncidentList');if(!metrics||!list)return;
  const scope=$('tpfIncidentVersion')?.value||'all',build=version();
  const visibleRows=rows.filter(item=>scope==='all'||(scope==='current'?sameBuild(item.app_version,build):scope==='unknown'?!item.app_version:!!item.app_version&&!sameBuild(item.app_version,build)));
  const active=visibleRows.filter(x=>x.status==='active').length,critical=visibleRows.filter(x=>x.status==='active'&&x.severity==='critical').length,resolved=visibleRows.filter(x=>x.status==='resolved').length;
  metrics.innerHTML=`<div><span>Activas</span><b>${active}</b></div><div class="critical"><span>Críticas</span><b>${critical}</b></div><div class="resolved"><span>Resueltas</span><b>${resolved}</b></div><div><span>Resultados filtrados (máx. 250)</span><b>${visibleRows.length}</b></div>`;
  if(!visibleRows.length){list.innerHTML='<div class="systemEmpty"><b>Sin incidencias en este periodo</b><span>Los errores nuevos aparecerán aquí automáticamente.</span></div>';return}
  list.innerHTML=`<table class="tpfIncidentTable"><thead><tr><th>Última detección</th><th>Módulo</th><th>Dispositivo</th><th>Incidencia</th><th>Severidad</th><th>Estado</th><th></th></tr></thead><tbody>${visibleRows.map(item=>`<tr><td data-label="Última detección"><b>${esc(fmt(item.last_seen_at))}</b><small>${Number(item.occurrences||1)>1?`${Number(item.occurrences)} veces`:''}</small></td><td data-label="Módulo">${esc(item.module||'General')}</td><td data-label="Dispositivo">${esc(sourceLabel(item.source))} · ${esc(item.device||'—')}</td><td data-label="Incidencia"><b>${esc(item.message)}</b>${item.detail||item.route?`<details><summary>Ver detalle técnico</summary><small>${esc(item.detail||'')}</small><small>${esc(item.route||'')}</small></details>`:''}${item.app_version?`<small>Versión: ${esc(item.app_version)}${sameBuild(item.app_version,version())?' · versión actual':' · otra versión'}</small>`:''}</td><td data-label="Severidad">${badge(item.severity,severityLabel(item.severity))}</td><td data-label="Estado">${badge(item.status,statusLabel(item.status))}</td><td><div class="tpfRowActions">${item.status==='active'?`<button class="tpfRowAction" data-system-resolve="${item.id}" type="button" title="Archiva el aviso; no repara el fallo">Marcar revisada</button>`:`<button class="tpfRowAction" data-system-reopen="${item.id}" type="button">Reabrir</button><button class="tpfRowAction danger" data-system-delete="${item.id}" type="button">Borrar</button>`}</div></td></tr>`).join('')}</tbody></table>`;
}
function followupLabel(value){return({offer_interest:'Oferta aceptada',offer_decline:'Oferta rechazada',offer_alternative:'Solicita otra oferta',response_cancelled:'Respuesta recibida',pre_send_blocked:'Envío bloqueado',verification_deferred:'Verificación aplazada',delivery_deferred:'Envío aplazado',followup_failed:'Fallo de seguimiento',followup_sent:'Recordatorio enviado'})[value]||value}
function followupResult(value){return({success:'Correcto',warning:'Aviso',error:'Error',info:'Información'})[value]||value}
function renderFollowups(){
  const metrics=$('tpfFollowupMetrics'),list=$('tpfFollowupList');if(!metrics||!list)return;
  const cancelled=followups.filter(x=>x.event_type==='response_cancelled'||x.event_type==='pre_send_blocked').length,deferred=followups.filter(x=>x.result==='warning').length,errors=followups.filter(x=>x.result==='error').length;
  metrics.innerHTML=`<div class="resolved"><span>Protegidos</span><b>${cancelled}</b></div><div><span>Aplazados</span><b>${deferred}</b></div><div class="critical"><span>Errores</span><b>${errors}</b></div><div><span>Eventos 7 días</span><b>${followups.length}</b></div>`;
  if(!followups.length){list.innerHTML='<div class="systemEmpty"><b>Sin eventos de seguimiento</b><span>Las próximas respuestas y comprobaciones aparecerán aquí.</span></div>';return}
  list.innerHTML=`<table class="tpfIncidentTable tpfFollowupTable"><thead><tr><th>Fecha</th><th>Cliente</th><th>Oferta</th><th>Acción</th><th>Resultado</th></tr></thead><tbody>${followups.map(item=>`<tr><td data-label="Fecha"><b>${esc(fmt(item.created_at))}</b></td><td data-label="Cliente"><b>${esc(item.client_name||'Cliente')}</b><small>${esc(item.opportunity_title||'')}</small></td><td data-label="Oferta">${esc([item.operator,item.offer_name].filter(Boolean).join(' · ')||'Oferta')}</td><td data-label="Acción"><b>${esc(followupLabel(item.event_type))}</b>${item.response_text?`<small>Cliente: “${esc(item.response_text)}”</small>`:''}${item.detail?`<small>${esc(item.detail)}</small>`:''}</td><td data-label="Resultado">${badge(item.result,followupResult(item.result))}</td></tr>`).join('')}</tbody></table>`;
  if($('tpfFollowupChecked'))$('tpfFollowupChecked').textContent=`Actualizado ${new Date().toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'})}`;
}
...
function renderAttention(){
 const host=$('tpfOperationalChecks');if(!host)return;
 let box=$('tpfSystemAttention');if(!box){box=document.createElement('div');box.id='tpfSystemAttention';box.className='systemCoverageNote';box.setAttribute('role','status');host.append(box)}
 const incidents=health?.incidents;
 if(!incidents){box.textContent='Incidencias centrales: pendientes de comprobar.';return}
 const total=Number(incidents.active||0),current=(incidents.by_version||[]).filter(x=>sameBuild(x.version,version())).reduce((n,x)=>n+Number(x.active||0),0);
 box.textContent=`${total} incidencias activas en el registro central · ${Number(incidents.critical||0)} críticas. ${current} registradas con esta versión. Las demás pueden proceder de otras versiones o no tener versión registrada. Consulta Incidencias para revisar su causa; un control técnico correcto no las resuelve.`;
 box.classList.toggle('tpfNeedsAttention',total>0);
}
function renderConsistency(){
 const host=$('tpfFollowupRegistry');if(!host)return;
 let box=$('tpfFollowupConsistency');if(!box){box=document.createElement('div');box.id='tpfFollowupConsistency';box.className='systemCoverageNote';host.querySelector('.systemStatusHead').after(box)}
 const checks=health?.followup_checks;
 if(!checks){box.textContent='Coherencia de ofertas: pendiente de comprobar.';return}
 const accepted=Number(checks.accepted_in_followup||0),closed=Number(checks.closed_pending_messages||0);
 box.innerHTML=`<b>Coherencia de ofertas y envíos</b><p>${accepted} ofertas aceptadas siguen en Seguimiento · ${closed} envíos pendientes vinculados a oportunidades perdidas o eliminadas.</p><small>Comprobado ${esc(fmt(health.checked_at))}. Este control detecta posibles incidencias; no cambia estados ni cancela envíos.</small>`+(checks.items?.length?`<details><summary>Ver casos que requieren revisión</summary><ul>${checks.items.map(x=>`<li>${esc(x.title||'Oportunidad eliminada')} — ${esc(x.reason)}</li>`).join('')}</ul></details>`:'');
 box.classList.toggle('tpfNeedsAttention',accepted+closed>0);
}
function renderHealth(){
  const grid=$('tpfOperationsGrid');if(!grid)return;if(!health?.checked_at){grid.innerHTML='<div class="systemEmpty">Servicios pendientes de comprobar. Pulsa Actualizar para reintentarlo.</div>';renderAttention();renderConsistency();return}const h=health||{},cron=h.cron||{},auto=h.automations||{},wa=h.whatsapp||{},gh=green.health||{},gs=green.state||{};
  ...
  ].join('');renderAttention();renderConsistency();if($('tpfOperationsChecked'))$('tpfOperationsChecked').textContent=h.checked_at?`Comprobado ${fmt(h.checked_at)}`:'Comprobación incompleta';
}
async function loadAll(){
  ...
  const days=Number($('tpfIncidentDays')?.value||7),status=$('tpfIncidentStatus')?.value||'active',search=$('tpfIncidentSearch')?.value?.trim()||'';
  try{
    const [eventsResult,followupResult,healthResult,greenHealthResponse,greenStateResponse]=await Promise.all([
      sb.rpc('crm_list_system_events',{p_days:days,p_limit:250,p_status:status,p_search:search}),
      sb.rpc('crm_list_offer_followup_events',{p_days:7,p_limit:100}),
      sb.rpc('crm_system_health_snapshot'),
      fetch('/api/green-health',{cache:'no-store'}),
      fetch('/api/green?action=state',{cache:'no-store'})
    ]);
    if(eventsResult.error)throw eventsResult.error;if(followupResult.error)throw followupResult.error;if(healthResult.error)throw healthResult.error;rows=eventsResult.data||[];followups=followupResult.data||[];health=healthResult.data||{};
    green.health=await greenHealthResponse.json().catch(()=>({ok:false}));green.state=await greenStateResponse.json().catch(()=>({ok:false}));renderRows();renderFollowups();renderHealth();
  }catch(error){health=null;renderHealth();if($('tpfOperationsChecked'))$('tpfOperationsChecked').textContent='No se pudo comprobar';if($('tpfIncidentList'))$('tpfIncidentList').innerHTML=`<div class="systemEmpty"><b>No se pudo cargar el registro central</b><span>${esc(error?.message||'Inténtalo de nuevo.')}</span></div>`;report({source:'health',module:'Estado del sistema',message:'No se pudo cargar el registro central',detail:error?.message||'',severity:'error'})}
...
async function cleanCentral(){if(!confirm('Se vaciará todo el historial de incidencias resueltas o ignoradas. Las incidencias activas y todos los datos del CRM se conservarán. ¿Continuar?'))return;const {data,error}=await sb.rpc('crm_clear_closed_system_events');if(error){$('tpfMaintenanceResult').textContent=error.message;return}$('tpfMaintenanceResult').textContent=`Historial cerrado vaciado: ${Number(data||0)} incidencia(s) técnica(s) eliminada(s).`;await loadAll()}
...
async function deleteEvent(id){if(!confirm('Esta incidencia técnica cerrada se borrará definitivamente. No se eliminará ningún dato del CRM. ¿Continuar?'))return;const {data,error}=await sb.rpc('crm_delete_closed_system_event',{p_id:Number(id)});if(error){alert(error.message);return}if(!data){alert('Solo se pueden borrar incidencias resueltas o ignoradas.');return}await loadAll()}
...
function bind(){
  document.addEventListener('click',event=>{
    const target=event.target?.closest?.('button,[data-system-resolve],[data-system-reopen],[data-system-delete]');if(!target)return;
    ...
    if(target.dataset.systemResolve&&confirm('Esto archiva el aviso como revisado; no corrige su causa. Si vuelve a ocurrir, aparecerá de nuevo. ¿Marcar como revisada?'))setStatus(target.dataset.systemResolve,'ignored');
    if(target.dataset.systemReopen)setStatus(target.dataset.systemReopen,'active');
    if(target.dataset.systemDelete)deleteEvent(target.dataset.systemDelete);
    if(target.closest?.('.nav[data-view="system"]'))setTimeout(loadAll,180);
  });
  document.addEventListener('change',event=>{if(event.target?.id==='tpfIncidentVersion')renderRows();if(['tpfIncidentDays','tpfIncidentStatus'].includes(event.target?.id))loadAll()});
