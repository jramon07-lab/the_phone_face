...
function renderPriority(d,map,pending){
  ...
  $('dashAlerts').innerHTML=page.length?`<table class="tdPriorityTable"><thead><tr><th scope="col">Cliente</th><th scope="col">Gestión / estado</th><th scope="col">Fecha de gestión</th><th scope="col">Próxima acción / WhatsApp</th><th scope="col"><span class="tdSrOnly">Acciones</span></th></tr></thead><tbody>${page.map(x=>`<tr>
    ...
    <td><button class="tdInterestButton" data-open="1" data-type="${x.type}" data-id="${esc(x.id)}">${esc(x.title)}</button><small class="tdLastActivity">${x.type==='opportunity'?esc(money(x.amount)):''}</small><span class="tdStatusPill ${x.tone}">${icon(x.stage==='Llamar'?'phone':x.tone==='green'?'checkCircle':x.tone==='amber'?'file':'refresh')}<span>${esc(x.stage)}</span></span></td>
