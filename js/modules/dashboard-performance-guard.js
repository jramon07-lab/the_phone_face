...
function ensureCss(){
 ...
 link.href='/assets/dashboard-home.css?v=20260919-sales-cockpit-1';document.head.appendChild(link);
}

function build(){
  const v=$('view-dashboard');if(!v||D.built)return;
  D.backupJson=$('backupJson');D.backupCsv=$('backupCsv');
  v.classList.add('tpfDashPro');v.dataset.homeVersion='20260919-sales-cockpit-1';
  v.innerHTML=`
  ...
  <section class="tdSalesHero"><div class="tdSalesHeroCopy"><span id="tdGreeting" class="tdHeroEyebrow">Tu día, más cerca del éxito</span><h1>Hoy comercial</h1><p>Llamadas, ofertas y clientes en movimiento.</p></div><div class="tdHeroFigures"><div><span>Total de oportunidades</span><b id="mOppTotal">—</b><small id="mOppAmount">—</small></div><div class="tdGoalRing"><b id="dashGoalProgress">—</b><span>objetivo</span></div><div class="tdHeroGoal"><span>Objetivo del mes</span><b id="dashWonAmount">—</b><small>de <strong id="dashGoalAmount">—</strong></small><button id="dashGoalEdit" class="tdHeroLink">Editar objetivo</button></div><button id="dashNewOpp" class="tdHeroNew">${icon('plus')} Nueva oportunidad</button><span id="dashForecastAmount" hidden></span></div></section>
  ...
  <details class="tdBusinessDetails"><summary><span>${icon('chart')} Analítica comercial</span><small>Embudo, previsión e indicadores ${icon('down')}</small></summary>
    <div class="tdBusinessContent"><div class="tdBusinessHead"><h2>Tu negocio en cifras</h2><button id="dashNewOppDetail" class="tdBtn primary">＋ Nueva oportunidad</button></div>
    <div class="tdMetrics">
...
function bind(){
  ...
  const openNewOpp=()=>{route('sales');setTimeout(()=>$('newOpp')?.click(),120)};$('dashNewOpp').onclick=openNewOpp;$('dashNewOppDetail').onclick=openNewOpp;
...
function render(){const d=D.data;if(!d)return;const map=new Map(d.stages.map(s=>[String(s.id),s])),won=d.opps.filter(o=>isWon(o,map)),open=d.opps.filter(o=>isOpen(o,map,d.today)),expired=d.opps.filter(o=>isExpired(o,map,d.today)),pending=d.tasks.filter(t=>status(t.status||'pending')==='pending'),todayTasks=pending.filter(t=>localDay(t.starts_at)===d.today),totalAmount=d.opps.reduce((n,o)=>n+Number(o.amount||0),0),conversion=d.opps.length?`${Math.round(won.length/d.opps.length*100)}%`:'0%';$('mOppTotal').textContent=d.opps.length;$('mOppAmount').textContent=money(totalAmount);[['tdMetricOppTotal',d.opps.length],['tdMetricOppAmount',money(totalAmount)],['tdMetricOppOpen',open.length],['tdMetricOppExpired',expired.length],['tdMetricTasks',pending.length],['tdMetricTasksToday',todayTasks.length?`${todayTasks.length} para hoy`:'Ninguna para hoy'],['tdMetricContacts',d.contacts],['tdMetricConversion',conversion]].forEach(([id,value])=>{$(id).textContent=value});renderPriority(d,map,pending);renderFunnel(d);renderCommercial(d,map,pending);renderGoal(d,won,open);renderForecast(d,map);renderUpcoming(d,map,pending);renderActivity();$('tdGreeting').textContent=Number(new Intl.DateTimeFormat('es-ES',{timeZone:'Europe/Madrid',hour:'numeric',hourCycle:'h23'}).format(new Date()))<14?'Buenos días':'Buenas tardes';$('tdToday').textContent=new Intl.DateTimeFormat('es-ES',{timeZone:'Europe/Madrid',weekday:'long',day:'numeric',month:'long'}).format(new Date());$('tdDataStatus').hidden=!d.warnings.length;$('tdDataStatus').textContent=d.warnings.length?'No se han podido cargar todos los datos. Pulsa Actualizar para volver a comprobarlos.':'';}
