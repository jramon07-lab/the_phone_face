...
function build(){
  ...
  <div class="tdCockpitGrid">
    <section class="tdCard tdPriorityCard">
      <div class="tdHead"><div class="tdTitleBlock">${icon('list')}<div><h2>Tu trabajo de hoy</h2><p id="tdPrioritySub">Clientes y oportunidades que requieren tu atención.</p></div></div><button id="tdAddContact" class="tdBtn outline" data-home-action="new-contact">＋ Añadir contacto</button></div>
  ...
  <details class="tdCommercialDetails"><summary>Resumen comercial y objetivo del mes</summary><section class="tdSalesHero" aria-label="Resumen comercial"><div class="tdHeroNumber"><span>Oportunidades abiertas</span><b id="mOppTotal">—</b><small>Con fecha vigente</small></div><div class="tdHeroNumber"><span>Importe total</span><b id="mOppAmount">—</b><small>Todas las oportunidades</small></div><div class="tdHeroNumber"><span>Vendido este mes</span><b id="dashWonAmount">—</b><small>Oportunidades ganadas</small></div><div class="tdHeroTarget"><div id="tdGoalRing" class="tdGoalRing"><b id="dashGoalProgress">—</b><span>objetivo</span></div><div class="tdHeroGoal"><span>Objetivo del mes</span><strong id="dashGoalAmount">—</strong><div class="tdGoalRail"><span id="tdGoalRailFill"></span></div><em id="tdGoalForecastLine">Calculando previsión…</em><button id="dashGoalEdit" class="tdHeroLink">Editar objetivo</button></div></div><span id="dashForecastAmount" hidden></span></section></details>
...
function renderPriority(d,map,pending){
  const all=priorityRows(d,map,pending),group=commercialGroups(d,map,pending).find(g=>g.key===D.filter),groupRows=group?group.rows:all,rows=workRows(D.filter==='followup'?(window.TPFOfferFollowup?.filterRows(groupRows,true)||groupRows):groupRows,D.query),pageSize=D.pageSize==='all'?Math.max(1,rows.length):Number(D.pageSize);
  ...
  saveWorkState();
  let followBar=$('ofHomeFilters');if(!followBar){followBar=document.createElement('div');followBar.id='ofHomeFilters';followBar.className='ofFilters';$('dashAlerts').before(followBar)}followBar.hidden=D.filter!=='followup';if(!followBar.hidden)followBar.innerHTML=window.TPFOfferFollowup?.controls(groupRows,true)||'';
  $('dashAlerts').innerHTML=page.length?`<table class="tdPriorityTable"><thead><tr><th scope="col">Cliente / gestión</th><th scope="col">Estado / fecha</th><th scope="col">Próxima acción / WhatsApp</th><th scope="col"><span class="tdSrOnly">Acciones</span></th></tr></thead><tbody>${page.map(x=>`<tr>
    <td>${x.contactId?`<button class="tdClientButton" data-open="1" data-type="contact" data-id="${esc(x.contactId)}" aria-label="Abrir contacto: ${esc(x.name)}">`:'<div class="tdClientButton" title="Sin contacto vinculado">'}<span class="tdAvatar">${esc(initials(x.name))}</span><span><b title="${esc(x.name)}">${esc(x.name)}</b><small>${esc(x.phone||'Sin teléfono')}${x.contactId?'':' · Sin vincular'}</small></span>${x.contactId?'</button>':'</div>'}<div class="tdOpportunityLine"><button class="tdInterestButton" data-open="1" data-type="${x.type}" data-id="${esc(x.id)}">${esc(x.title)}</button><small class="tdLastActivity">${x.type==='opportunity'?esc(money(x.amount)):''}</small></div></td>
    <td><span class="tdStatusPill ${x.tone}">${icon(x.stage==='Llamar'?'phone':x.tone==='green'?'checkCircle':x.tone==='amber'?'file':'refresh')}<span>${esc(x.stage)}</span></span><span class="tdNextAction ${x.expired?'isLate':''}">${icon('calendar')}<span>${esc(localDate(x.date))}${x.dateTime?'<small class="tdScheduledTime">'+(x.date===d.today?'Hoy · ':'')+esc(localTime(x.when))+'</small>':''}${x.expired?'<small>Vencida</small>':''}</span></span></td>
