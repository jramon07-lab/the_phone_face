...
function ensureCss(){
 ...
 link.href='/assets/dashboard-home.css?v=20260919-sales-cockpit-2';document.head.appendChild(link);
}

function build(){
  const v=$('view-dashboard');if(!v||D.built)return;
  D.backupJson=$('backupJson');D.backupCsv=$('backupCsv');
  v.classList.add('tpfDashPro');v.dataset.homeVersion='20260919-sales-cockpit-2';
  v.innerHTML=`
...
function renderPulse(d,map,pending,todayTasks){
  ...
  $('tdPulseTasks').textContent=todayTasks.length;
  $('tdPulseTaskText').textContent=todayTasks.length?'Pendientes para hoy':'Todo al día';
}
