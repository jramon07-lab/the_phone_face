...
function normalize(p={}){
  ...
  return {version:1,same,holder_name:same?'':nameCase(p.holder_name),holder_dni:same?'':clean(p.holder_dni).toUpperCase(),holder_phone:same?'':clean(p.holder_phone),recipient:!same&&p.recipient==='holder'?'holder':'contact'};
}
function validate(p){
  const x=normalize(p);
...
function read(id){
  ...
  return validate({same:get('same').checked,holder_name:get('holder_name').value,holder_dni:get('holder_dni').value,holder_phone:get('holder_phone').value,recipient:get('recipient').value});
}
...
.tpf-party-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.tpf-party-grid label:first-child{grid-column:1/-1}.tpf-party input:not([type=checkbox]),.tpf-party select{box-sizing:border-box;width:100%;min-width:0;height:40px;border:1px solid #cbd5e1;border-radius:8px;background:white;padding:8px 10px;color:#17243b;font:inherit}
