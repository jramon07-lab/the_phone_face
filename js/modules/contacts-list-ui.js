...
function renderList(){
 ...
 tbody.innerHTML=rows.map(r=>`<tr data-contact-id="${esc(r.id)}" class="${state.selected.has(r.id)?'selected':''}"><td><input class="tpfContactSelect" type="checkbox" data-id="${esc(r.id)}" ${state.selected.has(r.id)?'checked':''}></td><td><div class="tpfContactIdentity"><span class="tpfContactAvatar">${esc(initials(r))}</span><div><button class="tpfContactNameBtn" data-action="open" data-id="${esc(r.id)}">${esc(r.fullName)}</button>${window.TPFContactParty?.hint(r,state.rows)||''}${r.nickname?`<span class="tpfContactNickname">${esc(r.nickname)}</span>`:''}</div><button class="tpfContactPencil" data-action="open" data-id="${esc(r.id)}" title="Editar ficha">✎</button></div></td><td>${esc(r.dni||'—')}</td><td>${esc(r.phone||'—')}</td><td><span class="tpfSourceBadge">${esc(r.source==='BASE DE DATOS'?'Contactos':r.source)}</span></td><td>${labelsHtml(r.id)}</td><td class="tpfOppCell"></td><td class="tpfContactActionsCell"><div class="tpfContactActions"><button data-action="whatsapp" data-id="${esc(r.id)}" title="Enviar WhatsApp">◉</button><button data-action="schedule" data-id="${esc(r.id)}" title="Programar WhatsApp">◷</button><button data-action="tpf-contact-more" data-id="${esc(r.id)}" title="Acciones">•••</button></div></td></tr>`).join('');
 cards.innerHTML=rows.map(r=>`<article class="tpfContactCard ${state.selected.has(r.id)?'selected':''}" data-contact-id="${esc(r.id)}"><div class="tpfContactCardTop"><input class="tpfContactSelect" type="checkbox" data-id="${esc(r.id)}" ${state.selected.has(r.id)?'checked':''}><span class="tpfContactAvatar">${esc(initials(r))}</span><div class="tpfContactCardMain"><div class="tpfContactCardName"><button class="tpfContactNameBtn" data-action="open" data-id="${esc(r.id)}">${esc(r.fullName)}</button>${window.TPFContactParty?.hint(r,state.rows)||''}<button class="tpfContactPencil" data-action="open" data-id="${esc(r.id)}">✎</button></div>${r.nickname?`<span class="tpfContactNickname">${esc(r.nickname)}</span>`:''}<div class="tpfContactCardMeta"><span>DNI: ${esc(r.dni||'—')}</span><span>Teléfono: ${esc(r.phone||'—')}</span><span>Origen: ${esc(r.source==='BASE DE DATOS'?'Contactos':r.source)}</span></div><div class="tpfContactCardLabels">${labelsHtml(r.id)}</div><div class="tpfContactCardActions"><button class="secondary" data-action="whatsapp" data-id="${esc(r.id)}">◉ WhatsApp</button><button class="secondary" data-action="schedule" data-id="${esc(r.id)}">◷ Programar</button><button class="secondary" data-action="tpf-contact-more" data-id="${esc(r.id)}" title="Acciones">•••</button></div></div></div></article>`).join('');
