...
function buildUi(){
 ...
 const app=document.createElement('div');app.id='tpfContactsApp';app.className='tpfContactsApp';app.innerHTML=`
  ...
  <div class="tpfContactsBody">
   <aside id="tpfContactsFilters" class="tpfContactsFilters"><div class="tpfContactsFiltersHead"><b>Filtros</b><button id="tpfContactsFiltersClose" class="secondary tpfContactsMobileClose" aria-label="Cerrar filtros">×</button></div><label>Nombre y apellidos<input id="tpfFilterName"></label><label>DNI<input id="tpfFilterDni"></label><label>Teléfono<input id="tpfFilterPhone"></label><label>Origen<select id="tpfFilterSource"><option value="">Todos</option></select></label><label>Etiquetas<div class="tpfFilterLabelPicker"><button id="tpfFilterLabelToggle" type="button" class="tpfFilterLabelToggle" aria-expanded="false">Todas las etiquetas</button><div id="tpfFilterLabelPanel" class="tpfFilterLabelPanel hidden"><input id="tpfFilterLabelSearch" type="search" placeholder="Buscar etiqueta…"><div id="tpfFilterLabelChoices" class="tpfFilterLabelChoices"></div></div></div></label><section class="tpfOpportunityFilters"><b>Oportunidades</b><label>Estado<select id="tpfFilterOppStatus"><option value="">Todos</option><option value="open">Abiertas</option><option value="won">Ganadas</option><option value="lost">Perdidas</option></select></label><label>Cierre desde<input id="tpfFilterCloseFrom" type="date"></label><label>Cierre hasta<input id="tpfFilterCloseTo" type="date"></label></section><button id="tpfContactsClearFilters" class="secondary tpfClear">Limpiar filtros</button></aside>
 ...
 view.prepend(app);
 const selectedLabels=document.createElement('div');selectedLabels.id='tpfFilterSelectedLabels';selectedLabels.className='tpfFilterSelectedLabels';selectedLabels.setAttribute('aria-live','polite');byId('tpfFilterLabelPanel')?.insertAdjacentElement('afterend',selectedLabels);
...
function bindUi(){
 byId('tpfContactsFiltersClose').textContent='Cerrar';
 ...
 byId('tpfFilterLabelChoices').onclick=async e=>{const pick=e.target.closest('[data-label-id]');if(!pick)return;const id=pick.dataset.labelId||'';const selected=new Set(state.filters.labels||[]);if(!id)selected.clear();else if(selected.has(id))selected.delete(id);else selected.add(id);state.filters.labels=[...selected];state.page=1;if(state.filters.labels.length&&!state.labelsAllLoaded){setStatus('Cargando etiquetas de contactos…');await loadAllContactLabels();setStatus('');}renderLabelOptions();applyAndRender();};
...
function renderLabelOptions(){const choices=byId('tpfFilterLabelChoices'),toggle=byId('tpfFilterLabelToggle'),picked=byId('tpfFilterSelectedLabels'),selected=state.filters.labels||[];if(choices&&toggle){const q=norm(byId('tpfFilterLabelSearch')?.value||'');const rows=state.labels.filter(x=>!q||norm(labelName(x)).includes(q));choices.innerHTML=`<button type="button" class="tpfFilterLabelChoice ${!selected.length?'active':''}" data-label-id="">Todas las etiquetas</button>`+rows.map((x,i)=>`<button type="button" class="tpfFilterLabelChoice ${selected.includes(String(labelId(x)))?'active':''}" data-label-id="${esc(labelId(x))}"><span class="tpfLabelChip ${labelColor(i)}">${esc(labelName(x))}</span></button>`).join('');toggle.textContent=selected.length?`${selected.length} etiqueta${selected.length===1?'':'s'} elegida${selected.length===1?'':'s'}`:'Todas las etiquetas';}if(picked)picked.innerHTML=selected.map((id,i)=>{const row=state.labels.find(x=>String(labelId(x))===String(id));return row?`<button type="button" class="${labelColor(i)}" data-remove-label="${esc(id)}">${esc(labelName(row))} ×</button>`:'';}).join('');const box=byId('tpfCreateLabels');if(box)box.innerHTML=state.labels.length?state.labels.map((x,i)=>`<label class="tpfContactsLabelChoice"><input type="checkbox" value="${esc(labelId(x))}"><span class="tpfLabelChip ${labelColor(i)}">${esc(labelName(x))}</span></label>`).join(''):'<span class="small">No hay etiquetas creadas.</span>';byId('tpfContactsLabelsCount')&&(byId('tpfContactsLabelsCount').textContent=String(state.labels.length));window.TPFContactLabelPicker?.install(state.labels);}
...
function labelsHtml(id){const rows=state.labelsByContact.get(id);if(!rows)return '<span class="small">…</span>';if(!rows.length)return '<span class="small">Sin etiquetas</span>';return rows.map((x,i)=>`<span class="tpfLabelChip ${labelColor(i)}">${esc(labelName(x))}</span>`).join('');}
function renderList(){
...
function clearFilters(){state.filters={q:'',name:'',dni:'',phone:'',source:'',labels:[],oppStatus:'',closeFrom:'',closeTo:''};['tpfContactsSearch','tpfFilterName','tpfFilterDni','tpfFilterPhone','tpfFilterOppStatus','tpfFilterCloseFrom','tpfFilterCloseTo','tpfFilterLabelSearch'].forEach(id=>{if(byId(id))byId(id).value='';});if(byId('tpfFilterSource'))byId('tpfFilterSource').value='';byId('tpfFilterLabelPanel')?.classList.add('hidden');renderLabelOptions();state.page=1;applyAndRender();}
