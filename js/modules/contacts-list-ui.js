...
const state={rows:[],filtered:[],selected:new Set(),labels:[],labelsByContact:new Map(),labelsAllLoaded:false,labelsLoadPromise:null,opportunities:[],page:1,pageSize:25,loading:false,editingId:'',filters:{q:'',name:'',dni:'',phone:'',source:'',label:'',oppStatus:'',closeFrom:'',closeTo:''}};
...
function bindUi(){
 ...
 byId('tpfFilterLabelChoices').onclick=async e=>{const pick=e.target.closest('[data-label-id]');if(!pick)return;state.filters.label=pick.dataset.labelId||'';state.page=1;if(state.filters.label&&!state.labelsAllLoaded){setStatus('Cargando etiquetas de contactos…');await loadAllContactLabels();setStatus('');}byId('tpfFilterLabelPanel').classList.add('hidden');byId('tpfFilterLabelToggle').setAttribute('aria-expanded','false');renderLabelOptions();applyAndRender();};
...
function renderLabelOptions(){const choices=byId('tpfFilterLabelChoices'),toggle=byId('tpfFilterLabelToggle');if(choices&&toggle){const cur=state.filters.label,q=norm(byId('tpfFilterLabelSearch')?.value||'');const rows=state.labels.filter(x=>!q||norm(labelName(x)).includes(q));choices.innerHTML=`<button type="button" class="tpfFilterLabelChoice ${!cur?'active':''}" data-label-id="">Todas las etiquetas</button>`+rows.map((x,i)=>`<button type="button" class="tpfFilterLabelChoice ${String(labelId(x))===String(cur)?'active':''}" data-label-id="${esc(labelId(x))}"><span class="tpfLabelChip ${labelColor(i)}">${esc(labelName(x))}</span></button>`).join('');toggle.textContent=cur?labelName(state.labels.find(x=>String(labelId(x))===String(cur))):'Todas las etiquetas';}const box=byId('tpfCreateLabels');if(box)box.innerHTML=state.labels.length?state.labels.map((x,i)=>`<label class="tpfContactsLabelChoice"><input type="checkbox" value="${esc(labelId(x))}"><span class="tpfLabelChip ${labelColor(i)}">${esc(labelName(x))}</span></label>`).join(''):'<span class="small">No hay etiquetas creadas.</span>';byId('tpfContactsLabelsCount')&&(byId('tpfContactsLabelsCount').textContent=String(state.labels.length));window.TPFContactLabelPicker?.install(state.labels);}
