...
function applyFilters(){
 const f=state.filters,q=norm(f.q),name=norm(f.name),dni=norm(f.dni),phone=digits(f.phone);state.filtered=state.rows.filter(r=>{
  if(q&&!norm([r.fullName,r.dni,r.phone,r.email,r.bank,r.source,window.TPFContactParty?.search(r)].join(' ')).includes(q))return false;if(name&&!norm([r.fullName,r.data?.TPF_TITULAR?.holder_name].join(' ')).includes(name))return false;if(dni&&!norm([r.dni,r.data?.TPF_TITULAR?.holder_dni].join(' ')).includes(dni))return false;if(phone&&![r.phone,r.data?.TPF_TITULAR?.holder_phone].some(value=>digits(value).includes(phone)))return false;if(f.source&&r.source!==f.source)return false;if(f.label){const ls=state.labelsByContact.get(r.id)||[];if(!ls.some(x=>labelId(x)===f.label))return false;}return true;
