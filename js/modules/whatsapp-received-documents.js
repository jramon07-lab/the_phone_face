...
function decorate(){const box=$('waMessages');if(!box||!live()?.selected)return;const rows=history(),nodes=[...box.querySelectorAll('.waMsg')];nodes.forEach((node,index)=>{const m=rows[index],info=mediaInfo(m),id=safe(m?.idMessage);if(!m||!id||!incoming(m)||!eligible(info))return;const bubble=node.querySelector('.waBubble');if(!bubble||bubble.querySelector('[data-tpf-wa-document]'))return;const actions=document.createElement('div');actions.dataset.tpfWaDocument='1';actions.className='tpfWaDocumentActions';const menu=document.createElement('details'),summary=document.createElement('summary');menu.className='tpfWaSaveMenu';summary.textContent='Guardar…';menu.append(summary);const options=document.createElement('div');options.className='tpfWaDocMenu';options.append(button('Guardar en PC','pc',id,info),button('Guardar en documentos','drive',id,info));if(info?.kind==='image')options.append(button('PDF de DNI','dni',id,info));menu.append(options);actions.append(menu);bubble.append(actions);});}
