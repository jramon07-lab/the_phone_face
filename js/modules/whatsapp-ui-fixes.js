...
async function hydrateStatuses(force=false){
  if(!force&&Date.now()-lastStatusSweep<20000)return false;
  ...
  const batch=outs.filter(m=>{const id=String(m.idMessage),last=Number(statusChecked.get(id)||0);return !statusPending.has(id)&&(force||Date.now()-last>30000)}).slice(-4);
  if(!batch.length)return false;
  let changed=false;
  await Promise.all(batch.map(async m=>{const id=String(m.idMessage);statusPending.add(id);statusChecked.set(id,Date.now());try{const ctl=new AbortController();const timer=setTimeout(()=>ctl.abort(),4000);const r=await fetch('/api/green-status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({idMessage:id}),signal:ctl.signal});clearTimeout(timer);const j=await r.json().catch(()=>null);if(r.ok&&j?.status&&m.__realStatus!==j.status){m.__realStatus=j.status;changed=true}}catch(_){}finally{statusPending.delete(id)}}));
  return changed;
}
