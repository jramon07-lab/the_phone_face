...
const statusPending=new Set(),statusChecked=new Map();let lastStatusSweep=0;
async function hydrateStatuses(force=false){
  if(!force&&Date.now()-lastStatusSweep<60000)return false;
  lastStatusSweep=Date.now();
  let history=[];try{history=waLiveState?.history||[]}catch(_){return false}
  const outs=history.filter(m=>typeof waMessageDirection==='function'&&waMessageDirection(m)==='out'&&m?.idMessage&&!/read|played/i.test(String(m?.__realStatus||''))).slice(-12);
  const batch=outs.filter(m=>{const id=String(m.idMessage),last=Number(statusChecked.get(id)||0);return !statusPending.has(id)&&(force||Date.now()-last>60000)}).slice(-2);
  if(!batch.length)return false;
  let changed=false;
  await Promise.all(batch.map(async m=>{const id=String(m.idMessage);statusPending.add(id);statusChecked.set(id,Date.now());let timer;try{const ctl=new AbortController();timer=setTimeout(()=>ctl.abort(),8000);const r=await fetch('/api/green-status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({idMessage:id}),signal:ctl.signal});const j=await r.json().catch(()=>null);if(r.ok&&j?.status&&m.__realStatus!==j.status){m.__realStatus=j.status;changed=true}}catch(_){}finally{clearTimeout(timer);statusPending.delete(id)}}));
  return changed;
}
