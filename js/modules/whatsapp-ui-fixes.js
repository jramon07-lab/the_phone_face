...
function localPhone(raw){
  ...
  const local=p.startsWith('0034')?p.slice(4):p.startsWith('34')?p.slice(2):'';
  return /^[6789]\d{8}$/.test(local)?local:p;
}
function ensureStyles(){if($('tpfWaUiFixStyles'))return;const s=document.createElement('style');s.id='tpfWaUiFixStyles';s.textContent=`#waContactCard .pill.red,#waContactCard .pill.error,#waContactCard .danger,#waContactCard .error{background:#fee2e2!important;color:#7f1d1d!important;border-color:#fecaca!important}.waMsgMeta{display:flex!important;gap:6px!important;justify-content:flex-end!important;align-items:center!important;opacity:.8!important}.waReceipt{font-weight:700;letter-spacing:-1px}.waReceipt.read{color:#1d9bf0}#tpfWaCreateBack{position:fixed;inset:0;background:#0007;z-index:70000;display:flex;align-items:center;justify-content:center;padding:18px}#tpfWaCreateBack.hidden{display:none}#tpfWaCreateCard{width:min(760px,96vw);max-height:92vh;overflow:auto;background:#fff;border-radius:18px;box-shadow:0 24px 80px #0004;padding:20px}#tpfWaCreateGrid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.tpfWaField{display:flex;flex-direction:column;gap:6px}.tpfWaField.full{grid-column:1/-1}.tpfWaField input,.tpfWaField textarea{width:100%;box-sizing:border-box}.tpfWaField textarea{min-height:86px;resize:vertical}.tpfWaCreateHead{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:14px}.tpfWaCreateActions{display:flex;justify-content:flex-end;gap:10px;margin-top:16px}@media(max-width:700px){#tpfWaCreateGrid{grid-template-columns:1fr}}`;document.head.appendChild(s)}
...
function openCreateContact(){
 ...
 if(!chat||String(chat.id||'').includes('@g.us'))return;
...
const STATUS_SWEEP_MS=60000,STATUS_BATCH_SIZE=2;
const statusPending=new Set(),statusChecked=new Map();let lastStatusSweep=0,lastStatusChat='';
function statusCandidates(history,now=Date.now()){
  return (Array.isArray(history)?history:[])
    .filter(m=>typeof waMessageDirection==='function'&&waMessageDirection(m)==='out'&&m?.idMessage&&!/read|played/i.test(String(m?.__realStatus||'')))
    .filter(m=>{const id=String(m.idMessage),last=Number(statusChecked.get(id)||0);return !statusPending.has(id)&&now-last>=STATUS_SWEEP_MS})
    .sort((a,b)=>Number(statusChecked.get(String(a.idMessage))||0)-Number(statusChecked.get(String(b.idMessage))||0)||Number(waMessageTimestamp(b)||0)-Number(waMessageTimestamp(a)||0))
    .slice(0,STATUS_BATCH_SIZE);
}
async function hydrateStatuses(force=false){
  const now=Date.now();
  let chat='';try{chat=String(waLiveState?.selected?.id||'')}catch(_){}
  const changedChat=chat!==lastStatusChat;
  if(!force&&!changedChat&&now-lastStatusSweep<STATUS_SWEEP_MS)return false;
  let history=[];try{history=waLiveState?.history||[]}catch(_){return false}
  const batch=statusCandidates(history,now);
  if(!batch.length)return false;
  lastStatusSweep=now;lastStatusChat=chat;
  let changed=false;
  await Promise.all(batch.map(async m=>{const id=String(m.idMessage);statusPending.add(id);statusChecked.set(id,now);let timer;try{const ctl=new AbortController();timer=setTimeout(()=>ctl.abort(),8000);const r=await fetch('/api/green-status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({idMessage:id}),signal:ctl.signal});const j=await r.json().catch(()=>null);if(j?.throttled){statusChecked.delete(id);return}if(r.ok&&j?.status&&m.__realStatus!==j.status){m.__realStatus=j.status;changed=true}}catch(_){}finally{clearTimeout(timer);statusPending.delete(id)}}));
  return changed;
}
let painting=false;function paintMeta(){let rows=[];try{rows=[...(waLiveState?.history||[])].sort((a,b)=>Number(waMessageTimestamp(a)||0)-Number(waMessageTimestamp(b)||0))}catch(_){return}const nodes=[...document.querySelectorAll('#waMessages .waMsg')];nodes.forEach((node,i)=>{const m=rows[i];if(!m)return;const meta=node.querySelector('.waMsgMeta');if(!meta)return;const r=typeof waMessageDirection==='function'&&waMessageDirection(m)==='out'?receiptStatus(m):null;meta.innerHTML=`<span>${escText(dt(waMessageTimestamp(m)))}</span>${r?`<span class="waReceipt ${r.cls}" title="${r.title}">${r.txt}</span>`:''}`})}
function wrapRenderer(){if(typeof window.renderWaMessages!=='function'||window.renderWaMessages.__tpfUiFix)return false;const original=window.renderWaMessages;window.renderWaMessages=function(scrollBottom){original.call(this,scrollBottom);paintMeta();if(!painting){painting=true;hydrateStatuses(false).then(changed=>{if(changed)paintMeta()}).finally(()=>painting=false)}};window.renderWaMessages.__tpfUiFix=true;return true}
function forceCreateHandler(){window.createWaContact=openCreateContact;['waCreateContactTop','waSideCreateContact'].forEach(id=>{const el=$(id);if(!el)return;if(el.dataset.tpfCreateBound==='1')return;el.dataset.tpfCreateBound='1';el.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openCreateContact()},true)})}
function waViewVisible(){const v=$('view-whatsapplive');return !!v&&!v.classList.contains('hidden')}
function install(){ensureStyles();$('tpfWaCreateBack')?.remove();forceCreateHandler();if(!wrapRenderer()){let n=0;const t=setInterval(()=>{n++;forceCreateHandler();if(wrapRenderer()||n>80)clearInterval(t)},100)}document.addEventListener('click',e=>{const b=e.target?.closest?.('#waCreateContactTop');if(!b)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openCreateContact()},true);setInterval(()=>{if(!waViewVisible())return;forceCreateHandler();hydrateStatuses(false).then(ch=>{if(ch)paintMeta()})},60000)}
