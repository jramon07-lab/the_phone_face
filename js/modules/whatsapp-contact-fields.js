...
function row(id,host){const f=api().fields[id],r=document.createElement('div');r.className='waFieldRow';r.dataset.waField=id;const l=document.createElement('span');l.textContent=f.label;l.className='waFieldLabel';const value=document.createElement('span');value.className='waFieldValue';const copy=btn('⧉',async()=>{try{await navigator.clipboard.writeText(api().read(current()?.data,f));copy.textContent='✓';setTimeout(()=>copy.textContent='⧉',1200);}catch(_){copy.title='No se pudo copiar';}});copy.ariaLabel=copy.title='Copiar '+f.label;copy.className='waFieldCopy';r.append(l,value,copy,editButton(id));host.append(r);return r;}
