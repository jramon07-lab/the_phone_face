...
function edit(id){
 ...
 try{const r=await api().saveField({contactId:s.contactId,fieldId:id,original:s.original,value:valueOf(s)});if(String(current()?.id)===s.contactId)current().data=r.data;if(session===s)close();signature='';refresh();window.dispatchEvent(new CustomEvent('tpf:contact-updated',{detail:{id:s.contactId,previous:r.previous,data:r.data,phone:api().read(r.data,api().fields.contactPhone)}}));try{if(window.TPFContactsList?.invalidate)await window.TPFContactsList.invalidate();else await window.tpfReloadContacts?.();}catch(_){}
