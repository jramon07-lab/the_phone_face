...
let epoch=0,id='',link=null,status=null,files=[],next='',folder=null,busy=false,reconnectNeeded=false;
...
function render(){
 ...
 ${status?.canManage&&connected&&reconnectNeeded?'<button type="button" data-doc-reconnect>Volver a conectar Google Drive</button>':''}
 ${status?.canManage&&connected?'<button type="button" data-doc-choose>'+(link?'Cambiar carpeta':'Vincular carpeta existente')+'</button>':''}
 ${!link&&connected&&status?.canUpload?'<button type="button" data-doc-ensure>Crear / vincular carpeta</button>':''}
 ${link&&connected?'<button type="button" data-doc-refresh>Actualizar archivos</button>':''}
 ${connected&&status?.canUpload?'<button type="button" data-doc-upload '+(link&&!folder?.canUpload?'disabled':'')+'>Subir PDF / fotos</button><button type="button" data-doc-scan '+(link&&!folder?.canUpload?'disabled':'')+'>Fotos / cámara a PDF</button><input type="file" data-doc-file hidden accept="application/pdf,image/jpeg,image/png,image/webp,image/heic,image/heif" multiple>':''}</div>
 ...
 host.querySelector('[data-doc-ensure]')?.addEventListener('click',async()=>{const e=epoch;if(await prepareFolder()&&e===epoch)await loadFiles(false);});
 host.querySelector('[data-doc-scan]')?.addEventListener('click',async()=>{const e=epoch;if(await prepareFolder()&&e===epoch)scanDocument();});
 host.querySelector('[data-doc-upload]')?.addEventListener('click',()=>host.querySelector('[data-doc-file]').click());
 host.querySelector('[data-doc-file]')?.addEventListener('change',e=>{const selected=[...e.target.files];e.target.value='';upload(selected);});
}

async function prepareFolder(){
 if(busy)return false;if(link&&folder?.canUpload)return true;
 const target=id,e=epoch,c=current(),snapshot=JSON.stringify(c?.data||{});let ready=false;
 const check=()=>{if(epoch!==e||id!==target||current()?.id!==target||JSON.stringify(c?.data||{})!==snapshot)throw Error('La ficha cambió. Vuelve a abrir Documentos.');};
 await run(async()=>{
  check();if(!window.TPFDocumentFolderAuto)throw Error('Actualiza la página para preparar la carpeta.');
  const result=await window.TPFDocumentFolderAuto.ensure({contactId:target,data:c.data,check,notice:message});check();
  c.data=result.data;link=result.link;folder=result.folder;files=[];next='';render();ready=!!folder.canUpload;
  message(ready?'Carpeta preparada: '+link.folder_name+'.':'Google no permite subir archivos a esta carpeta.');
 });
 return ready&&epoch===e&&current()?.id===target;
}

async function upload(selected){const e=epoch;if(selected.length&&await prepareFolder()&&e===epoch)await uploadPrepared(selected);}
...
async function run(fn){if(busy)return;const runEpoch=epoch;busy=true;host.setAttribute('aria-busy','true');const buttons=[...host.querySelectorAll('button')].map(b=>[b,b.disabled]);buttons.forEach(([b])=>b.disabled=true);try{await fn();}catch(e){if(runEpoch===epoch){if(/Vuelve a conectar Google Drive/i.test(String(e?.message||''))){reconnectNeeded=true;render();}message(e.message);}}finally{busy=false;host.removeAttribute('aria-busy');buttons.forEach(([b,disabled])=>{if(b.isConnected)b.disabled=disabled;});}}
