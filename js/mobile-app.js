(function(){
  ...
  async function enter(user){
    ...
    try{
      ...
      await client.auth.signOut({scope:'local'}).catch(()=>{});state.user=null;showLogin(error?.message||'No se pudieron cargar tus permisos.');
    }
  }
  ...
  function render(){
    ...
    try{
      switch(current.parts[0]){
        ...
        case 'opportunity':view.innerHTML=renderOpportunity(current.parts[1]);loadMobileOpportunityFollowup(current.parts[1]);break;
  ...
  function renderContact(id){
    ...
    return `<div class="m-page m-contact-profile">${pageHead('Contacto','contacts',has('can_edit_records')?`<button class="m-back" data-action="route" data-route="edit-contact/${esc(id)}" aria-label="Editar contacto">✎</button>`:'')}
      <section class="m-contact-identity"><div class="m-avatar">${esc(initials(contact))}</div><div><h1>${esc(contact.fullName)}</h1>${contact.nickname?`<p class="m-contact-nickname">${esc(contact.nickname)}</p>`:''}<p>${esc(contact.phone||'Sin teléfono')} · ${esc(contact.dni||'Sin DNI')}</p></div></section>
  ...
  function contactPhoneActions(contact){
    ...
    const call=`<a id="mobileContactCall" class="m-secondary" href="tel:+${esc(phones[0].number)}">Llamar</a>`;
    return phones.length>1?`<div class="m-contact-phone-actions"><label class="m-field"><span>Elegir teléfono para WhatsApp</span><select id="mobileContactPhone" class="m-select" data-contact-phone="${esc(contact.id)}">${phones.map(p=>`<option value="${p.number}">${esc(p.label)}</option>`).join('')}</select></label>${call}</div>`:`<div class="m-contact-phone-actions">${call}</div>`;
  }
  ...
  function renderOpportunity(id){
    ...
    const stage=state.board.stages.find(row=>String(row.id)===String(opp.stage_id));
    return `<div class="m-page">${pageHead('Oportunidad','opportunities')}<div class="m-profile-hero"><div class="m-avatar">◇</div><h1>${esc(opp.title||'Oportunidad')}</h1><p>${esc(opp.client_name||'Sin contacto')}</p></div><div class="m-info-card">${infoRow('Titular',mobileOpportunityIdentity(opp).holder)}${infoRow('DNI / NIF',mobileOpportunityIdentity(opp).dni)}${infoRow('Gestionado por',mobileOpportunityIdentity(opp).manager||'El propio titular')}${infoRow('Columna / Estado',stage?.name||'—')}${infoRow('Importe',opp.amount!=null?money(opp.amount):'—')}${infoRow('Cierre previsto',date(opp.expected_date))}${infoRow('Teléfono',opp.phone)}${infoRow('Notas',opp.notes)}</div><div id="mobileOpportunityFollowup" class="m-info-card" data-opportunity-id="${esc(id)}" style="display:none;margin-top:12px"></div>${has('can_edit_sales')?`<div class="m-detail-actions"><button class="m-primary" data-action="route" data-route="edit-opportunity/${esc(id)}">Editar oportunidad</button><button class="m-danger" data-action="profile-delete-opportunity" data-id="${esc(id)}">Eliminar oportunidad</button></div>`:''}${opp.record_id?`<button class="m-secondary" style="width:100%;margin-top:12px" data-action="route" data-route="contact/${esc(opp.record_id)}">Ver contacto</button>`:''}</div>`;
  }
  ...
  async function loadMobileOpportunityFollowup(id){
    const target=byId('mobileOpportunityFollowup');if(!target||!client)return;
    const expected=String(id),{data,error}=await client.rpc('crm_offer_followup_latest',{p_opportunity_id:id});
    if(route().parts[0]!=='opportunity'||route().parts[1]!==expected||target.dataset.opportunityId!==expected)return;
    if(error){target.style.display='block';target.innerHTML=infoRow('Seguimiento','No se pudo consultar el registro');return}
    if(!data){target.style.display='none';target.innerHTML='';return}
    const when=data.created_at?new Date(data.created_at).toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'}):'—',reply=data.response_text?` · Cliente: “${data.response_text}”`:'';
    target.style.display='block';target.innerHTML=infoRow('Seguimiento',`${mobileFollowupLabel(data.event_type)}${reply}`)+infoRow('Última comprobación',when)+infoRow('Movimiento de columna','No automático');
  }
