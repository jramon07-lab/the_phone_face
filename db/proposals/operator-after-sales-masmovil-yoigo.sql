-- Posventa completa de MásMóvil y Yoigo.
...
begin
 annual_review:=right(lower(coalesce(new.trigger_config->>'automation_code','')),14)='_annual_review';
...
do $$
declare
 target record;uid uuid;tramitado_id uuid;proximo_id uuid;
 day_template_id bigint;security_template_id bigint;day_rule_id uuid;security_rule_id uuid;review_rule_id uuid;
 day_body text;security_body text;review_title text;
begin
 ...
 for target in select * from (values ('MásMóvil','masmovil'),('Yoigo','yoigo')) as operators(operator,code) loop
   day_body:=format($body$Hola {nombre} 👋
...
📦 Las instrucciones para devolver el router anterior pueden tardar hasta 15 días.$body$);
   security_body:=format($body$Hola {nombre} 👋 ¿Qué tal va todo con %s?
...
📱 Si te llaman ofreciendo un terminal gratis o financiado, consúltanos antes. Pueden aplicarte permanencias elevadas de 24, 36 o 48 meses.$body$,target.operator);
   review_title:='REVISIÓN '||upper(target.operator);

   for uid in select distinct user_id from public.crm_automations where lower(coalesce(trigger_config->>'automation_operator',''))=lower(target.operator) loop
     select id into day_template_id from public.wa_templates
      where user_id=uid and lower(btrim(name))=lower(target.operator||' · instalación y devolución de router') order by created_at limit 1;
     if day_template_id is null then
       insert into public.wa_templates(user_id,name,body,category,shortcut)
       values(uid,target.operator||' · Instalación y devolución de router',day_body,target.operator,null) returning id into day_template_id;
     else
       update public.wa_templates set body=day_body,category=target.operator,updated_at=now() where id=day_template_id;
     end if;

     select id into security_template_id from public.wa_templates
      where user_id=uid and lower(btrim(name))=lower(target.operator||' · seguimiento de seguridad a los 3 meses') order by created_at limit 1;
     if security_template_id is null then
       insert into public.wa_templates(user_id,name,body,category,shortcut)
       values(uid,target.operator||' · Seguimiento de seguridad a los 3 meses',security_body,target.operator,null) returning id into security_template_id;
     else
       update public.wa_templates set body=security_body,category=target.operator,updated_at=now() where id=security_template_id;
     end if;

     select id into day_rule_id from public.crm_automations
      where user_id=uid and (trigger_config->>'automation_code'=target.code||'_day_one'
       or (lower(name)=lower('TRAMITACIÓN · '||target.operator) and trigger_config->>'automation_code' is null))
      order by (trigger_config->>'automation_code'=target.code||'_day_one') desc,created_at limit 1;
     if day_rule_id is null then
       ...
       values(uid,'TRAMITACIÓN · '||target.operator,true,'opportunity_stage',
        jsonb_build_object('stage_id',tramitado_id,'automation_operator',target.operator,'automation_category','Tramitación','automation_code',target.code||'_day_one'),
        'flow_v1',jsonb_build_object('version',1,'lifecycle',jsonb_build_object('mode','after_sale','version',1),'steps',jsonb_build_array(
          ...
          jsonb_build_object('kind','action','action_type','send_template','config',jsonb_build_object('template_id',day_template_id::text))))) returning id into day_rule_id;
     else
       update public.crm_automations set name='TRAMITACIÓN · '||target.operator,enabled=true,trigger_type='opportunity_stage',
        trigger_config=jsonb_build_object('stage_id',tramitado_id,'automation_operator',target.operator,'automation_category','Tramitación','automation_code',target.code||'_day_one'),
        action_type='flow_v1',action_config=jsonb_build_object('version',1,'lifecycle',jsonb_build_object('mode','after_sale','version',1),'steps',jsonb_build_array(
          jsonb_build_object('kind','action','action_type','record_sale_month','config',jsonb_build_object()),
          jsonb_build_object('kind','wait','unit','days','value',1,'business_schedule','phone_house'),
          jsonb_build_object('kind','action','action_type','send_template','config',jsonb_build_object('template_id',day_template_id::text)))),updated_at=now() where id=day_rule_id;
     end if;

     select id into security_rule_id from public.crm_automations where user_id=uid and trigger_config->>'automation_code'=target.code||'_security_3_months' limit 1;
     if security_rule_id is null then
       insert into public.crm_automations(user_id,name,enabled,trigger_type,trigger_config,action_type,action_config)
       values(uid,'POSVENTA · '||target.operator||' · 3 meses',true,'opportunity_stage',
        jsonb_build_object('stage_id',tramitado_id,'automation_operator',target.operator,'automation_category','Posventa','automation_code',target.code||'_security_3_months'),
        'flow_v1',jsonb_build_object('version',1,'lifecycle',jsonb_build_object('mode','after_sale','version',1),'steps',jsonb_build_array(
          jsonb_build_object('kind','action','action_type','record_sale_month','config',jsonb_build_object()),
          jsonb_build_object('kind','wait','unit','months','value',3,'business_schedule','phone_house'),
          jsonb_build_object('kind','action','action_type','send_template','config',jsonb_build_object('template_id',security_template_id::text))))) returning id into security_rule_id;
     else
       update public.crm_automations set name='POSVENTA · '||target.operator||' · 3 meses',enabled=true,trigger_type='opportunity_stage',
        trigger_config=jsonb_build_object('stage_id',tramitado_id,'automation_operator',target.operator,'automation_category','Posventa','automation_code',target.code||'_security_3_months'),
        action_type='flow_v1',action_config=jsonb_build_object('version',1,'lifecycle',jsonb_build_object('mode','after_sale','version',1),'steps',jsonb_build_array(
          jsonb_build_object('kind','action','action_type','record_sale_month','config',jsonb_build_object()),
          jsonb_build_object('kind','wait','unit','months','value',3,'business_schedule','phone_house'),
          jsonb_build_object('kind','action','action_type','send_template','config',jsonb_build_object('template_id',security_template_id::text)))),updated_at=now() where id=security_rule_id;
     end if;

     select id into review_rule_id from public.crm_automations where user_id=uid and trigger_config->>'automation_code'=target.code||'_annual_review' limit 1;
     if review_rule_id is null then
       insert into public.crm_automations(user_id,name,enabled,trigger_type,trigger_config,action_type,action_config)
       values(uid,'RENOVACIÓN · '||target.operator||' · 11 meses',true,'opportunity_stage',
        jsonb_build_object('stage_id',tramitado_id,'automation_operator',target.operator,'automation_category','Renovaciones','automation_code',target.code||'_annual_review'),
        'flow_v1',jsonb_build_object('version',1,'lifecycle',jsonb_build_object('mode','after_sale','version',1),'steps',jsonb_build_array(
          jsonb_build_object('kind','action','action_type','record_sale_month','config',jsonb_build_object()),
          jsonb_build_object('kind','action','action_type','set_review_date','config',jsonb_build_object('value',1,'unit','years')),
          jsonb_build_object('kind','wait','unit','months','value',11,'business_schedule','phone_house'),
          jsonb_build_object('kind','action','action_type','prepare_operator_review','config',jsonb_build_object('stage_id',proximo_id,'title',review_title))))) returning id into review_rule_id;
     else
       update public.crm_automations set name='RENOVACIÓN · '||target.operator||' · 11 meses',enabled=true,trigger_type='opportunity_stage',
        trigger_config=jsonb_build_object('stage_id',tramitado_id,'automation_operator',target.operator,'automation_category','Renovaciones','automation_code',target.code||'_annual_review'),
        action_type='flow_v1',action_config=jsonb_build_object('version',1,'lifecycle',jsonb_build_object('mode','after_sale','version',1),'steps',jsonb_build_array(
          jsonb_build_object('kind','action','action_type','record_sale_month','config',jsonb_build_object()),
          jsonb_build_object('kind','action','action_type','set_review_date','config',jsonb_build_object('value',1,'unit','years')),
          jsonb_build_object('kind','wait','unit','months','value',11,'business_schedule','phone_house'),
          jsonb_build_object('kind','action','action_type','prepare_operator_review','config',jsonb_build_object('stage_id',proximo_id,'title',review_title)))),updated_at=now() where id=review_rule_id;
     end if;
   end loop;
 end loop;
end $$;
