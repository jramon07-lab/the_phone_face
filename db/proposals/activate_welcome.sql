...
do $$ declare aid uuid;lid uuid;uid uuid:=auth.uid();begin
 ...
 if exists(select 1 from public.crm_welcome_settings) then raise exception 'La bienvenida ya está configurada: revisar antes de cambiar';end if;
 ...
 insert into public.crm_welcome_settings(id,enabled,label_id,automation_id) values(true,true,lid,aid);
end $$;
commit;
