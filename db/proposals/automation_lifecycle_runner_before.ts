...
async function complete(job:any,status:"done"|"failed",errorMessage?:string,extra:any={}){await sb.from("crm_server_automation_jobs").update({status,error_message:errorMessage||null,completed_at:status==="done"?new Date().toISOString():null,updated_at:new Date().toISOString()}).eq("id",job.id);await sb.from("crm_automation_runs").insert({automation_id:job.automation_id,user_id:job.user_id,event_key:job.event_key,context:errorMessage?{...(job.context||{}),...extra,error:errorMessage,server:true}:{...(job.context||{}),...extra,server:true},status:status==="done"?"ok":"error"});}
async function requeue(job:any,message:string,minutes=2){await sb.from("crm_server_automation_jobs").update({status:"pending",error_message:message,run_at:new Date(Date.now()+minutes*60000).toISOString(),updated_at:new Date().toISOString()}).eq("id",job.id);}
...
async function shouldSkip(job:any){const a=job.action_config||{},ctx=job.context||{};if(a.__flow_guard==="no_response"&&await hasResponseSince(ctx))return "El cliente respondió: condición no cumplida";if(a.__stop_if_response&&await hasResponseSince(ctx))return "El cliente respondió: repetición detenida";return "";}
...
async function expandFlow(job:any){const steps=Array.isArray(job.action_config?.steps)?job.action_config.steps:[];if(!steps.length)throw new Error("El flujo no tiene pasos");const origin=eventBase(job.context||{});const started=origin.toISOString();const root=`${job.event_key}:flow`;let delay=0;let guard="";let previous:any=null;let actionNo=0;
  for(let i=0;i<steps.length;i++){const s=steps[i]||{};
    ...
    if(s.kind==="repeat"){
      ...
      for(let k=1;k<=times;k++){delay+=every;const cfg={...previous.config,__flow_guard:previous.guard||null,__stop_if_response:!!s.stop_if_response,__flow_repeat:k};const ctx={...(job.context||{}),flow_root:root,flow_started_at:started};await enqueueChild(job,`${root}:repeat:${previous.baseIndex}:${k}`,previous.type,cfg,ctx,new Date(origin.getTime()+delay).toISOString());}
    }
  }
  return {children:actionNo,flow_root:root};
}

async function updateSiblingOpportunity(job:any,oppId:string){const root=String(job.context?.flow_root||"");if(!root)return;const {data}=await sb.from("crm_server_automation_jobs").select("id,context").eq("automation_id",job.automation_id).eq("status","pending").contains("context",{flow_root:root});for(const row of data||[]){await sb.from("crm_server_automation_jobs").update({context:{...(row.context||{}),opportunity_id:oppId}}).eq("id",row.id);}}

async function processJob(job:any){const a=job.action_config||{},ctx=job.context||{};try{
    ...
    if(job.action_type==="create_task"){
      ...
      const row:any={pipeline_id:stage.pipeline_id,stage_id:stage.id,record_id:ctx.contact_id||null,title:vars(a.title||"Oportunidad",ctx),client_name:vars(a.client_name||ctx.name||"",ctx)||null,phone:vars(a.phone||ctx.phone||"",ctx)||null,amount:numberValue(a.amount,ctx),expected_date:expected,owner_user_id:userValue(a.owner_user_id,job),status:a.status||"open",notes:vars(a.notes||"",ctx)||null,position:Number(a.position||0)};
      const {data:opp,error}=await sb.from("sales_opportunities").insert(row).select("id").single();if(error)throw error;
      const custom=a.custom_values&&typeof a.custom_values==="object"?a.custom_values:{};const rows=Object.entries(custom).filter(([,v])=>String(v??"").trim()!=="").map(([field_id,v])=>({opportunity_id:opp.id,field_id,value:vars(String(v),ctx)}));if(rows.length){const {error:e2}=await sb.from("sales_custom_values").upsert(rows,{onConflict:"opportunity_id,field_id"});if(e2)throw e2;}await updateSiblingOpportunity(job,opp.id);
