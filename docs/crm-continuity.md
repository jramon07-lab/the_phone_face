...
- Checkpoint GitHub del código: `a758b65c3da958b6f2d7e457e9ea8f23305d1b01`. Actions de desarrollo `34337382819`, correcta. Publicado después en la rama estable sin cambiar su alias; Vercel `dpl_91ikG2svQCPpJeF4KW6rrs2Q2QHc`, READY, destino inmutable `the-phone-face-app-whatsapp-fotos-y-multimedia-midalzb3w.vercel.app`.
- Validación posterior sobre el despliegue estable: Actions `34338013565`, correcta. Los workflows modulares `34337994150` y `34337994162` también terminaron correctamente.
- Edge Function `crm-automation-runner` versión 11 activa con autenticación propia mediante `x-tpf-cron-secret`; `verify_jwt=false` se conserva porque el cron no usa una sesión de usuario y la función valida el secreto contra `crm_check_runner_secret` antes de reclamar trabajos.
- Prueba real de respuesta: el trabajo sintético `0aaec8b6-7efa-4956-9c39-e5783f5baa58` quedó cancelado, la respuesta se sincronizó y el texto prohibido no apareció en el historial saliente.
- Prueba real de botones al único número autorizado: trabajo `4e8abcf9-82bb-4f52-a730-b98983a09577`, `done`, un intento, sin error. GREEN devolvió un mensaje saliente `typeMessage=interactiveButtons`, sin usar el respaldo numerado.
