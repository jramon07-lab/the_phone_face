...
const uiSource = sourceFiles.filter(fs.existsSync).map(file => fs.readFileSync(file, 'utf8')).join('\n');
...
if (fs.existsSync('index.html')) {
  ...
  if (/<style(?:\s|>)/i.test(html)) { console.error('MODULAR_GUARD_FAIL: index.html vuelve a contener CSS inline'); failed=true; }
  else console.log('MODULAR_GUARD_OK: index.html sin CSS inline');
  if (/<script(?![^>]*\bsrc=)[^>]*>[\s\S]*?<\/script>/i.test(html)) { console.error('MODULAR_GUARD_FAIL: index.html vuelve a contener JavaScript inline'); failed=true; }
  else console.log('MODULAR_GUARD_OK: index.html sin JavaScript inline');
  if (html.includes('/js/app-core.js')) { console.error('MODULAR_GUARD_FAIL: index.html todavía carga app-core.js monolítico'); failed=true; }
  ...
  if(splitPos<0||runtimePos<0||splitPos>runtimePos){ console.error('MODULAR_GUARD_FAIL: orden físico/runtime inválido'); failed=true; }
...
if (fs.existsSync('api/green.js')) {
  ...
  for (const marker of ['sendMessage', 'getStateInstance', 'getSettings']) {
    if (!green.includes(marker)) { console.error(`MODULAR_GUARD_FAIL: GREEN sin ${marker}`); failed = true; }
    else console.log(`MODULAR_GUARD_OK: GREEN conserva ${marker}`);
  }
}
...
if (fs.existsSync('api/index.js')) {
  ...
  if(!indexApi.includes('VERCEL_GIT_COMMIT_SHA') || !indexApi.includes("path.join(__dirname,'..','index.html')")){
    console.error('MODULAR_GUARD_FAIL: api/index.js no sirve el commit desplegado/local index'); failed=true;
  }else console.log('MODULAR_GUARD_OK: api/index.js usa el índice del despliegue actual');
}

for(const [file,legacyMarker] of [['api/index-clean.js','tpf-wa-templates-tdz-fix'],['api/index-fix.js','tpf-crm-automations-tdz-fix']]){
  if(fs.existsSync(file)){
    const src=fs.readFileSync(file,'utf8');
    if(src.includes(legacyMarker)){ console.error(`MODULAR_GUARD_FAIL: ${file} conserva parche legado ${legacyMarker}`); failed=true; }
