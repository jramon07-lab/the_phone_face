...
(async()=>{
 ...
 const events=[];const ctx={window:{dispatchEvent:e=>events.push(e.detail)},CustomEvent:function(type,options){this.detail=options.detail;},crmGetContactLabels:async id=>{assert.equal(id,'original-contact');return labels.map(id=>({id}));},sb:{rpc:async(name,args)=>{assert.equal(name,'crm_set_contact_labels');calls.push(JSON.parse(JSON.stringify(args)));if(fail)return{error:new Error('denied')};labels=Array.from(args.p_label_ids);return{error:null};}}};
 ...
 assert.equal(events.length,2);assert.equal(events[0].contactId,'original-contact');assert.deepEqual(Array.from(events[1].labels,x=>x.id),['b','new','a']);
