...
assert.deepEqual(T.parseCallbackData('tpf:task:snooze:15m:a1'),{action:'snooze',value:'15m',taskId:'a1'});
assert.deepEqual(T.parseCallbackData('tpf:task:reopen:a1'),{action:'reopen',value:'',taskId:'a1'});
...
const callable=T.initialKeyboard('a1',{phone:'600 000 001',baseUrl:'https://crm.example'}).inline_keyboard.flat();
assert.deepEqual(callable.map(button=>button.text),['✅ Completar','📞 Llamar','⏰ Posponer','📅 Cambiar fecha']);
assert.equal(callable.find(button=>button.text==='📞 Llamar').url,'https://crm.example/api/telegram-call?phone=%2B34600000001');
assert.deepEqual(T.reopenKeyboard('a1').inline_keyboard.flat(),[{text:'↩️ Reactivar tarea',callback_data:'tpf:task:reopen:a1'}]);
assert.deepEqual(T.postponeKeyboard('a1').inline_keyboard.flat().map(button=>button.text),['En 15 minutos','En 30 minutos','En 1 hora','En 2 horas','↩️ Volver']);
...
assert.equal(T.postponedAt('15m',new Date('2026-09-20T08:00:00.000Z').getTime()),'2026-09-20T08:15:00.000Z');
assert.equal(T.postponedAt('30m',new Date('2026-09-20T08:00:00.000Z').getTime()),'2026-09-20T08:30:00.000Z');
assert.equal(T.postponedAt('2h',new Date('2026-09-20T08:00:00.000Z').getTime()),'2026-09-20T10:00:00.000Z');
