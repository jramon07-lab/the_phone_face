...
assert.match(html, /<th scope="col">Fecha prevista<\/th>/);
assert.match(html, /<th scope="col" class="tdAmountHeading">Importe<\/th>/);
assert.match(html, /class="tdAmount">34,99\s*€/);
assert.match(html, /Actualizada: 19\/09\/2026 · 11:15/);
assert.match(html, /class="tdNextAction [^"]*"[\s\S]*?19\/09\/2026/);
renderPriority({ opps: [{ ...opp, amount: 0 }], today: date }, map, []);
assert.match(nodes.get('dashAlerts').innerHTML, /class="tdAmount">0,00\s*€/);
renderPriority({ opps: [], today: date }, map, [{ id: 'demo-task', title: 'Tarea sintética', starts_at: '2026-09-19T09:30:00Z', status: 'pending' }]);
assert.match(nodes.get('dashAlerts').innerHTML, /class="tdNoAmount" aria-label="Sin importe">—/);
