...
assert.match(inline,/function rowMatchesChat\(row,\s*chat\)/,'every delayed sidebar result must validate its contact against the current chat');
assert.match(inline,/token\s*!==\s*waRefreshToken\s*\|\|\s*!waContextCurrent\(expected\)/,'an older sidebar refresh must not repaint a newer conversation');
assert.match(inline,/recordId:\s*safe\(row\.id\)/,'a delayed Google lookup must retain the CRM-record identity');
assert.match(inline,/requireRecord:\s*true/,'a delayed Google lookup must require the same CRM record');
...
assert.match(inline,/TPF_WHATSAPP_CHAT_ID:\s*chatId|d\.TPF_WHATSAPP_CHAT_ID\s*=\s*chatId/,'the confirmed chat-to-contact binding must persist for every PC');
assert.match(inline,/safe\(saved\.recordId\)\s*===\s*bound/,'persisted WhatsApp display names must belong to the exact saved binding');
assert.match(inline,/!rowConfirmedForChat\(row,\s*chat\)[\s\S]*?clearWhatsappNicknames\(\)[\s\S]*?return\s*["']{2}/,'unconfirmed contacts must clear the previous nickname without replacing their names');
