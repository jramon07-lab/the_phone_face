...
assert.match(source,/crm_create_offer_execution_v10/);
...
const managerRecipientSql=fs.readFileSync(path.join(root,'supabase/migrations/20260918130000_manager_recipient_for_offers.sql'),'utf8');
assert.match(managerRecipientSql,/crm_create_offer_execution_v9/);
const immediateOfferSql=fs.readFileSync('supabase/migrations/20260918133000_offer_dispatch_immediate.sql','utf8');
assert.match(immediateOfferSql,/crm_create_offer_execution_v10/);
assert.match(immediateOfferSql,/crm_private\.dispatch_runner_now/);
assert.match(managerRecipientSql,/El destinatario no gestiona este contacto/);
assert.match(managerRecipientSql,/recipient_contact_id/);
