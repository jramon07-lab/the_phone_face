...
assert.equal(ctx.executionActionLabel({status:'error'}),'Ver incidencia →');
assert.equal(ctx.executionActionLabel({status:'ok'}),'Ver pasos →');
assert.match(source,/data-ac-exec="\$\{esc\(x\.key\)\}">\$\{executionActionLabel\(x\)\}/);
assert.match(source,/Ver pasos →/);
console.log('PASS customer grouping: linked identity, separate sales, incident detail routing, no 100-row truncation');
