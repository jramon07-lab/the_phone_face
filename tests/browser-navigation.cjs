...
(async()=>{
 ...
 const shared=fixture(true);let sharedDirty=false;
 shared.window.TPFContactEditor={isDirty:()=>sharedDirty,owns:el=>el.id==='sharedInput'};
 const sharedInput=shared.node('sharedInput');await shared.emit('focusin',{target:sharedInput});sharedInput.value='Cancelled field edit';
 let sharedBlocked=false;await shared.emit('beforeunload',{preventDefault(){sharedBlocked=true;}});
 assert.equal(sharedBlocked,false,'Shared editor owns its baseline and can restore individual edits');
 sharedDirty=true;await shared.emit('beforeunload',{preventDefault(){sharedBlocked=true;}});
 assert.equal(sharedBlocked,true,'Real changes from the shared editor must protect navigation');
 shared.confirm=false;
 const restore=shared.node('restoreNote');restore.textContent='Cancelar edición';restore.matches=s=>s.includes('[data-note-restore]');
 let restoreBlocked=false;await shared.emit('click',{target:restore,preventDefault(){restoreBlocked=true;}});
 assert.equal(restoreBlocked,false,'Cancelling a note edit must not ask to leave or discard the whole form');
 const sharedCancel=shared.node('tpfContactsCreateCancel');sharedCancel.textContent='Cancelar';
 await shared.emit('click',{target:sharedCancel,preventDefault(){restoreBlocked=true;}});
 assert.equal(restoreBlocked,true,'Closing the whole shared form still requires handling unsaved changes');
