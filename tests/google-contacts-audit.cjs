...
assert.match(html,/google-contacts-audit\.js\?v=/,'debe cargar el comparador');
assert.match(source,/padding:14px 0 42px/,'el botón debe quedar separado de la insignia flotante de pruebas');
assert.match(html,/runtime\.js\?v=/,'debe cargar los módulos de búsqueda');
assert.match(fs.readFileSync('js/modules/runtime.js','utf8'),/contacts-list-ui\.js/,'debe cargar el buscador de PC');
assert.match(fs.readFileSync('movil/index.html','utf8'),/mobile-app\.js\?v=/,'debe cargar el buscador móvil');
...
assert.match(source,/async function run\(\)/,'la comparación debe ejecutarse como una fase independiente');
assert.match(source,/trashCrm|trashGoogle|mergeDuplicateCrm/,'las acciones posteriores deben ser explícitas y separadas de la comparación');
...
assert.doesNotMatch(source,/Aplicar cambios automáticos/,'la comparación no debe aplicar cambios automáticos');
