...
assert.match(runner,/check=\(\)=>offer\?hasOfferDecision\(ctx,secret\):hasResponseSince\(ctx,secret\)/,'offer reminders must stop only after an explicit button decision');
assert.match(runner,/a\.__flow_guard==="no_response"&&await check\(\)/,'pre-send guard must remain for offer decisions and ordinary automations');
