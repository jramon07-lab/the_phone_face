...
const cards=['completed','completed','pending','pending','pending','pending','pending'].map(card);cards.forEach(c=>c.classList.owner=c);
...
context.filterTasks();assert.deepEqual(cards.map(c=>c.filtered),[true,true,false,false,false,false,true]);assert.equal(buttons[0].textContent,'Pendientes (5)');assert.equal(context.taskEmpty.hidden,true);
context.selected='tareas';context.filterTasks();assert.deepEqual(cards.map(c=>c.filtered),[true,true,false,false,false,false,false]);
context.taskFilter='completed';context.filterTasks();assert.deepEqual(cards.map(c=>c.filtered),[false,false,true,true,true,true,true]);assert.equal(buttons[1]['aria-pressed'],'true');
