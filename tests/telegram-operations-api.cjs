...
const failed={id:'f1',action_type:'__send_whatsapp',status:'failed',action_config:{template_id:676},context:{contact_name:'Luis',phone:'611111111'},error_message:'Proveedor no disponible',updated_at:new Date(fixedNow-2000).toISOString()};
...
const telegramCalls=[],templateQueries=[];
global.fetch=async(url,options={})=>{
  ...
  if(path.startsWith('wa_templates?')){templateQueries.push(path);assert(path.includes('id=in.(676)'));return response(200,[{id:676,name:'Saludo',body:'Hola desde plantilla'}]);}
  if(path.startsWith('whatsapp_templates?')){templateQueries.push(path);assert(path.includes('id=in.(11111111-2222-3333-4444-555555555555)'));return response(200,[{id:'11111111-2222-3333-4444-555555555555',name:'Legado',body:'Legacy'}]);}
...
(async()=>{
  const templates=await handler._test.templateMap([676,'676','11111111-2222-3333-4444-555555555555','bad', '9223372036854775808', '1),id.neq.0'].map(template_id=>({action_config:{template_id}})));
  assert.equal(templates.size,2);assert.equal(templates.get('676').body,'Hola desde plantilla');assert.equal(templateQueries.length,2);
  templateQueries.length=0;
  assert.equal((await handler._test.templateMap([])).size,0);assert.equal(templateQueries.length,0);
  ...
  assert(telegramCalls[1].text.includes('Hola desde plantilla'));
