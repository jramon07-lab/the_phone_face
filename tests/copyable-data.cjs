...
const context={window:{addEventListener:(name,handler)=>{callbacks[name]=handler}},document,navigator:{clipboard:{async writeText(value){copied=value}}},MutationObserver:class{observe(){}},setTimeout(){}};
const source=fs.readFileSync('js/modules/copyable-data.js','utf8').replace('window.TPFCopyData={valueOf,empty,copy}','window.TPFCopyData={valueOf,empty,copy,attach}');
...
(async()=>{
 ...
 assert.equal(copied,'600000002','Copy reads the current value, not the old rendered value');assert(stopped&&prevented,'Copy must not also open a row or submit its form');assert.equal(button.dataset.copied,'true');
