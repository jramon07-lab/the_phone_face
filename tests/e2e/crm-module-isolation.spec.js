...
test('runtime: un fallo aislado no rompe el CRM', async ({ page }) => {
  ...
  expect(result.runtime).toBe(true);expect(result.appVisible).toBe(true);expect(result.isolated).toBe(true);expect(result.hasError).toBe(true);
  await page.locator('.nav[data-view="sales"]').click();await expect(page.locator('#view-sales')).toBeVisible();
});
...
test('módulo Ficha de contacto: protección, observaciones, etiquetas y apertura ágil', async ({ page }) => {
  ...
  const visibleMs=await page.evaluate(()=>performance.now()-window.__tpfOpenStart);
  expect(visibleMs,'La ficha tarda demasiado en hacerse visible').toBeLessThan(3500);
  ...
  await expect(page.locator('#contactObservations')).toBeDisabled();
  await page.locator('#tpfContactEditToggle').click();
  await expect(page.locator('#contactPhone')).toBeEnabled();await expect(page.locator('#contactObservations')).toBeEnabled();await expect(page.locator('#contactSave')).toBeVisible();
