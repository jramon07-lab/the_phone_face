...
test('estructura física: cada dominio carga desde su archivo y el index no usa app-core', async ({ page }) => {
  await page.goto('/', { waitUntil: 'domcontentloaded' });
  const structure=await page.evaluate(() => ({scripts:[...document.scripts].map(s=>s.getAttribute('src')||'').filter(Boolean).map(s=>new URL(s,location.href).pathname),styles:[...document.querySelectorAll('link[rel="stylesheet"]')].map(x=>new URL(x.getAttribute('href')||'',location.href).pathname)}));
...
test('módulo Ficha de contacto: editor separado protegido, actividad y WhatsApp interno', async ({ page }) => {
  ...
  await expect(tabs.filter({hasText:'Oportunidades'})).toBeVisible();
  await expect(tabs.filter({hasText:'Tareas'})).toBeVisible();
  await expect(page.locator('#cpAddNote')).toHaveText('Ir a Notas');
  await tabs.filter({hasText:'Tareas'}).click();
  await tabs.filter({hasText:'Todos'}).click();
  await page.locator('#cpAddNote').click();
  await expect(page.locator('#cpRefTab-notas')).toHaveAttribute('aria-selected','true');
  await expect(page.locator('#cpNotesTitle')).toBeVisible();
  await page.locator('#cpNotesCancel').click();
