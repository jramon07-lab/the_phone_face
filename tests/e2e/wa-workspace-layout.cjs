...
(async()=>{
 const browser=await chromium.launch({executablePath:process.env.CHROMIUM_PATH||'/tmp/chromium',args:['--no-sandbox','--disable-dev-shm-usage','--disable-gpu','--no-zygote','--single-process']});
 ...
 await page.click('[data-wa-side-tab="client"]');await page.click('[data-wa-edit="contactPhone"]');await page.locator('.waFieldEditor input').fill('600000002');await page.click('[data-wa-side-tab="work"]');await page.click('[data-wa-side-tab="client"]');assert.equal(await page.locator('.waFieldEditor input').inputValue(),'600000002');await page.click('[data-wa-field-save]');assert.equal(await page.evaluate(()=>waLiveState.contact.data.contactPhone),'600000002');
