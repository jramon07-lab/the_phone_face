...
(async()=>{
  ...
  context.fetch=async()=>Response.json({ok:true,degraded:true,rateLimited:true,messages:[]});
  await assert.rejects(()=>context.guard.providerResponsesSince({phone:'695661409'},'2026-09-07T09:10:00Z','runner-secret'),e=>e.responseCheck===true,'degraded empty history must never mean no reply');
  context.fetch=async()=>Response.json({ok:true});
  await assert.rejects(()=>context.guard.providerResponsesSince({phone:'695661409'},'2026-09-07T09:10:00Z','runner-secret'),e=>e.responseCheck===true,'malformed history must defer too');
