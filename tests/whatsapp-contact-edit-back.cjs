...
assert.match(connectorSource,/TPFContactsList\?\.edit/);
assert.match(connectorSource,/await window\.TPFContactsList\.edit\(c\.id\)/);
...
assert.match(indexSource,/runtime\.js\?v=20260911-search-debounce-1/);
...
(async()=>{
 ...
 const window={TPFModules:{register:(_,module)=>module.install()},addEventListener:(type,fn)=>{listeners[type]=fn}};
 ...
 const click=id=>listeners.click({target:{closest:selector=>selector.split(',').includes('#'+id)?{}:null}});
