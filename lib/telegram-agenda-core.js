...
function stableOrigin(value){
  const raw=clean(value)||'https://the-phone-face-app-whatsapp-fotos-y.vercel.app';
  let url;try{url=new URL(raw)}catch(_){throw new Error('CRM_STABLE_ORIGIN debe ser un origen HTTPS sin ruta, credenciales ni parámetros.');}
  if(!/^https:\/\/[^\s/?#\\]+\/?$/i.test(raw)||url.protocol!=='https:'||url.username||url.password||url.pathname!=='/'||url.href.includes('?')||url.href.includes('#')){
    throw new Error('CRM_STABLE_ORIGIN debe ser un origen HTTPS sin ruta, credenciales ni parámetros.');
  }
  return url.origin;
}
...
function parseCallbackData(value){
  const match=/^tpf:task:(complete|reopen|postpone|date|menu|snooze|day):(?:(15m|30m|1h|2h|[1-7]):)?([0-9a-z-]{1,40})$/i.exec(clean(value));
...
function callLink(phone,baseUrl){
  const number=normalizedPhone(phone),base=clean(baseUrl).replace(/\/$/,'');
  return number&&/^https:\/\//i.test(base)?`${base}/api/telegram-call?phone=${encodeURIComponent(number)}`:'';
}
function initialKeyboard(taskId,{phone='',baseUrl=''}={}){
  const rows=[
    [{text:'✅ Completar',callback_data:callbackData(taskId,'complete')}]
  ];
  const url=callLink(phone,baseUrl);
  if(url)rows.push([{text:'📞 Llamar',url}]);
  rows.push([
...
function reopenKeyboard(taskId){
  return {inline_keyboard:[
    [{text:'↩️ Reactivar tarea',callback_data:callbackData(taskId,'reopen')}]
  ]};
}
function postponeKeyboard(taskId){
  return {inline_keyboard:[
    [{text:'En 15 minutos',callback_data:callbackData(taskId,'snooze','15m')}],
    [{text:'En 30 minutos',callback_data:callbackData(taskId,'snooze','30m')}],
    [{text:'En 1 hora',callback_data:callbackData(taskId,'snooze','1h')}],
    [{text:'En 2 horas',callback_data:callbackData(taskId,'snooze','2h')}],
...
function postponedAt(choice,now=Date.now()){
  if(choice==='15m')return new Date(now+15*60000).toISOString();
  if(choice==='30m')return new Date(now+30*60000).toISOString();
  if(choice==='1h')return new Date(now+3600000).toISOString();
  if(choice==='2h')return new Date(now+2*3600000).toISOString();
  return'';
}
...
module.exports={DEFAULT_DELIVERY_WINDOW_MS,stableOrigin,deliveryKey,notificationMoments,dueDeliveries,deliverySignature,madridDateTime,recordDni,taskMessage,callbackData,parseCallbackData,callbackTaskId,normalizedPhone,callLink,initialKeyboard,reopenKeyboard,postponeKeyboard,dateKeyboard,postponedAt,moveToMadridDay,webhookSecret};
