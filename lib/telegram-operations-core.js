...
function contactLink(contactId,baseUrl){const id=clean(contactId),base=clean(baseUrl).replace(/\/$/,'');return id&&/^https:\/\//i.test(base)?`${base}/?contact=${encodeURIComponent(id)}`:''}
function businessMessage(event,{baseUrl=''}={}){
  const payload=event?.payload||{},titles={offer_sent:'💰 Oferta enviada',offer_accepted:'✅ Me interesa',offer_declined:'❌ No me interesa',offer_decline_reason:'📝 Motivo de rechazo',offer_alternative_requested:'🔄 Quiere otra oferta',sale_processed:'📄 Venta tramitada',sale_won:'🏆 Venta ganada',sale_lost:'❌ Venta perdida',offer_cancelled:'🛑 Oferta cancelada'};
  ...
  if(clean(payload.reason))lines.push(`Motivo: ${clean(payload.reason)}`);
  if(clean(payload.task_date))lines.push(`Nueva oferta: tarea para ${clean(payload.task_date)}`);
  const link=contactLink(payload.contact_id,baseUrl);if(link)lines.push(`Ficha del cliente: ${link}`);
  return lines.join('\n');
}
function followupMessage(event,offer={},opportunity={},options={}){
  const type=clean(event?.event_type),payload=event?.payload||{},titles={followup_sent:'🔄 Seguimiento enviado',verification_deferred:'⏳ Seguimiento aplazado',delivery_deferred:'⏳ Seguimiento aplazado',pre_send_blocked:'⏸ Seguimiento detenido',followup_paused:'⏸ Seguimiento pausado',followup_resumed:'▶️ Seguimiento reactivado',offer_response_ignored:'ℹ️ Respuesta no aplicada'};
  ...
  if(clean(payload.response))lines.push(`Respuesta: ${clean(payload.response)}`);
  if(clean(payload.stage_name))lines.push(`Estado actual: ${clean(payload.stage_name)}`);
  if(clean(event?.detail))lines.push(`Detalle: ${clean(event.detail)}`);
  const link=contactLink(payload.contact_id||opportunity.record_id,options.baseUrl);if(link)lines.push(`Ficha del cliente: ${link}`);
  return lines.join('\n');
}
...
module.exports={DELIVERY_WINDOW_MS,SEND_ACTIONS,madridDay,madridDateTime,manualAt,dueManual,deliveryKey,manualMessage,jobMessage,failureMessage,contactLink,businessMessage,followupMessage,automaticJobs,dailySummary,shouldSendDaily};
